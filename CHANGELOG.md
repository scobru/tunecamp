...
- **The federated catalog no longer advertises paid releases as free to take.** Closing the download routes (previous entry) stopped this instance handing out a release it sells, but said nothing to the rest of the federation: `/api/catalog/full` published every public release with no indication of whether its file may actually leave, so a remote Network page or a Sidecamp client offered a Download button on all of them — and, for cross-instance grabs, reached for `/api/tracks/:id/stream`, which is public by design and returns the original bytes. The catalog now carries a `downloadable` flag on every release and track, evaluated as an anonymous stranger (the caller a federated fetch actually is, regardless of who triggered it locally) and aware of per-release price overrides a client cannot see. `catalog-cache.service` turns that into a `downloadUrl` only where the remote allows it — pointing at the gated `/api/tracks/:id/download` rather than `/stream`, so a release that goes on sale later starts refusing instead of quietly handing over the master — and mirrors the peer-track branch that has always withheld the URL when `allowDownload` is false. An instance older than the flag is still respected: it publishes its release's mode and price, which is enough to run the same policy locally. Streaming is untouched on every path; only the file transfer is withheld. `GET /api/tracks` gained the same flag per track (evaluated for the viewer, so a curator or the owning artist sees their own catalog as downloadable), and the `v_tracks` view finally selects `albums.download` and the album price columns — `Track.album_download` and `album_price` had been declared and mapped into every track DTO since they were added, but nothing ever selected them, so they were always `undefined`.

- **A release on sale is no longer downloadable straight past the store.** The paywall lived in the webapp, which merely hides the download button unless `albums.download === 'free'`. The server's own routes — `GET /api/tracks/:id/download`, `GET /api/albums/:id/download` and `GET /api/releases/:id/download` — guarded nothing but *visibility* (`canConsumeTrack`, or the album's private/public flag), and `/api/tracks` is mounted with `optionalAuth`, so a bare `curl https://instance/api/tracks/123/download` handed an anonymous caller the master file of a release the artist was selling. The same routes ignored the other two distribution modes: **Streaming Only** (`none`/`null`) and **External Showcase** (`external`) both served the full audio to anyone. Sidecamp's Network tab surfaced this as a plain Download button on every track of every instance it could see, but the hole was in the API, not in Sidecamp. All three routes now run one shared policy (`src/server/common/download-access.ts`) on top of the visibility check: `free` stays public; anything carrying a price or set to `codes` requires an unlock code matching the track or its release, a purchase recorded against the account, or an active subscription — the same entitlements `GET /api/payments/download/:trackId` has always accepted — and answers `402` otherwise; streaming-only and external releases answer `403`. Curators, Managers and Root Admins, the owning account and the linked artist bypass the gate, so staff and the seller still reach their own masters, and a track priced on its own is gated even when its release is not (with the per-release price override honoured, exactly as checkout resolves it). Instances that never sold anything are unaffected: a track with no release container, or a release explicitly marked **Free Download**, downloads as before.

- **An SSO token can no longer name one identity key for verification and a different one for the account lookup.** A FID token carries the identity twice — `masterKeySource.pubKey` and the flat `zenPubKey` — and both arrive in the same request body. `fid` verified the signature against the first while `POST /api/auth/zen/sso` selected the account with `ssoToken.zenPubKey`, exactly as fid's own documentation instructs. An attacker needed nothing but the victim's *public* key, which every SSO token publishes: sign a payload naming your own keypair, put the victim's key in `zenPubKey`, omit the passport (fid only checks it `if (token.passport)`), and this route minted a full session for the victim — administrators included. Fixed upstream in [fid 4.0.1](https://github.com/scobru/fid/pull/1), which now refuses a token whose two copies disagree; this route no longer depends on that check to be sound. It resolves the account from `masterKeySource.pubKey` when present — the copy the signature actually covers — and refuses a mismatched token itself (`400 SSO token identity mismatch`), so an older or patched-out `fid` cannot reopen the hole. `__mocks__/fid.ts` is a faithful port of upstream and reproduced the same key selection verbatim, which is why the suite could never have caught this; it carries the same change, and the regression test builds the forged token end to end against the real route.

- **A future-dated SSO token no longer authenticates forever.** fid's age check was one-sided, so a token whose `issuedAt` lay ahead of the verifier's clock had a negative age and never expired. Fixed upstream and mirrored in the test double; `POST /api/auth/zen/sso` now answers `400` for one.

- **The iframe embed no longer publishes the private library.** `/embed/track/:id` and `/embed/release|album/:idOrSlug` are mounted with no auth middleware — by design, since an embed is an iframe on someone else's page and carries no credential — but the router read rows straight out of the database with no visibility check of any kind. Walking the id space returned the title, artist, cover art and full tracklist of anything on the instance, including drafts and private-library material: `albums.visibility` defaults to `private` and `albums.status` to `draft`, so the *default* state of a new album was publicly embeddable. `/api/tracks/:id/stream` was still gating the audio, so what leaked was metadata rather than the recordings. Every embed now resolves against a guest `ViewerContext`: an album or release goes through `VisibilityGuardian.isItemVisible`, a track through `canConsumeTrack` (so a private album's track still embeds where a public playlist carries it, exactly as the stream route allows), and the tracks listed under a published release are filtered individually rather than trusted because their parent passed. Anything a guest may not see answers 404, not 403 — a 403 confirms the id exists, which is the same leak in miniature.

- **A Subsonic token no longer acts as whatever `u=` claims.** `/rest` accepts a JWT or `tc_` API token in the `p=` parameter, and that path deliberately took the username from the verified token payload rather than the `u=` query parameter, which it never validates. Ten lines further down the middleware overwrote it: `req.user = { username: u, ... }` ran unconditionally for every request that got past authorization. So a valid token for any account, plus `u=<victim>`, acted as the victim across every per-user Subsonic surface — playlists created under their name, their starred items, ratings, bookmarks and play queue read and written — and, on `getPodcasts`/`getNewestPodcasts`, which derive a `ViewerContext` by looking that username up in the `admin` table, with the victim's role. The password and token+salt paths verify `u=` against the account they authenticate, so those keep it; the token paths now keep the account the token names.

- **Subsonic playlist routes now check ownership.** `deletePlaylist.view` deleted any playlist by bare id, and `updatePlaylist.view` renamed, re-scoped and added or removed tracks on any playlist by bare id, with no check at all — any authenticated Subsonic client could destroy or rewrite every playlist on the instance, and flip a private one public. `getPlaylist.view` returned any playlist's name and contents the same way. The REST twins under `/api/playlists` have always enforced `playlist.username !== req.username` → 403; the Subsonic routes were added later and never picked it up. One `requirePlaylistAccess` helper now gates all three: owner or admin for writes, and additionally any public playlist for reads. A playlist the caller may not see reports as not found rather than forbidden, so the id space is not an enumeration oracle.

- **On-chain direct payments now require proof you sent the transaction.** `/api/payments/verify` checked recipient and amount for wallet-to-wallet purchases (no checkout contract involved) but never the sender, so anyone who spotted a qualifying tx on a block explorer could submit its hash before the real buyer and claim their download code — the tx is public the moment it's broadcast. The endpoint now requires a wallet signature over a challenge binding the trackId and txHash, checked against the transaction's `from` address; `CheckoutModal.tsx` prompts for it right after the on-chain tx confirms. See finding #11 in [security-review-payments.md](docs/security-review-payments.md).

- **A demoted account no longer keeps its old privileges until its token expires.** The derivation of a request's identity was copied across all six guards and the copies had drifted: `requireAdmin`, `requireUser` and `optionalAuth` re-read the account row and preferred the database's role, while `requireManager` and `requireRootAdmin` trusted the role claim inside the JWT and never consulted the database at all. Since `updateAdmin` changed a role without invalidating outstanding tokens, and `verifyToken` re-reads only `token_version` and `is_active`, a demoted root admin went on passing the guards protecting `MANAGE_SYSTEM` — the least protected path was the most privileged one. All six guards now derive identity through one module (`middleware/identity.ts`), the account row wins over the token claim, and `updateAdmin` revokes that account's tokens when the role actually changes (an artist-link or storage-quota edit does not log anyone out). Two smaller divergences went with it: the account is resolved by id rather than through `getUserByUsername`, which matches `username COLLATE NOCASE OR alias COLLATE NOCASE` and could land on a different account; and an account deactivated in the database is now refused rather than merely flagged `isActive: false` while still being admitted.
...
- **An upload now says how far along it is, and can be started on its own.** Saving a release fires a chain of requests — release metadata, cover art, then the audio, then the per-track edits — and the audio is the slow half, tens of megabytes a file. All the artist got was a disabled Save button: no way to tell an upload in flight from a stuck one, on the one action of the whole app that routinely takes minutes. Queued audio now sits in the tracklist with its size, the toolbar grows an **Upload N files** button that sends the audio without saving-and-closing the release (creating the release first if it does not exist yet, since a file needs somewhere to land), and a bar under the toolbar reports the whole chain: which file is going up, how far its bytes have got, and how much of the batch is left. Each queued row carries its own bar, so one stalled file is visible rather than averaged away. Audio goes up one request per file instead of one request for the batch — the browser reports bytes for a request as a whole, so a single 400 MB request can only ever say "somewhere between 0 and 100%", and one failure partway through took every file with it; per file, a failure costs only that file and the rest stay queued for a retry. Save and Publish end in a toast rather than in silence. The same bar reached the Upload Tracks and Upload Sample dialogs: the tracks dialog counted finished files, so a single large file sat at 0% until it was completely done, and now weights progress by bytes and leaves the finished bar next to the result.

- **A public playlist can now be read without an account.** `GET /api/playlists/:id` is members-only, so a playlist marked public was visible to the rest of the federation by name alone: the community player could list one and never play it, and the same gap kept a listener from importing their own public playlists into a remote player. The new `GET /api/playlists/:id/public` answers anonymously for a playlist already marked public — 404 for anything else, so whether a private playlist exists is not something a stranger can probe. It is deliberately narrow: a purpose-built response rather than the internal row (no file paths, no per-user `starred`/`rating` fields), and every track still goes through `canConsumeTrack` as a guest, so a private album's track appears only where being in this public playlist is what makes it consumable. Wildcard CORS is opted into by that one path, and never for a request carrying a cookie or `Authorization` header.
- **Network DIG**: crate digging now also works over this TuneCamp network's own users, not just Bandcamp's fan-collector graph. From a release in your own library, `POST /api/dig/run` with `source: "network"` finds who else on this instance (and, opt-in, on federated peers) starred it, and what else those users starred — ranked by co-occurrence, aggregate counts only, never usernames. Correlation uses a release's `external_id` when it has one (Bandcamp/MusicBrainz match) or a normalized title+artist fallback, so a TuneCamp-only release with no external match is still included. Peers answer via the new public `POST /api/community/dig-lookup`, gated behind the `digNetworkOptIn` setting (off by default) and filtered to public/released catalog items only. See [FEDERATION.md](docs/FEDERATION.md#network-dig-cross-instance-discovery).
...
- **The primary admin of an older installation is root admin again.** `is_root` means "row 1" everywhere in this codebase, but installations created before the `root_admin` role exists store that same account with `role = 'admin'`. Login has always reconciled the two (`if (user.id === 1) userRole = ROOT_ADMIN`, so the token says root_admin); the request path does not, and since identity derivation started re-reading the account row and preferring it over the token claim, the row's `admin` won. `MANAGE_SYSTEM` is granted by role alone, so the owner of such an instance got `403` from every root-only surface — `GET /api/admin/settings` among them, which is what the admin area reports as not being signed in as root admin — while the same request reported `isRootAdmin: true`. Three fixes, because the disagreement had three ways in: identity derivation now resolves a root account's role to `root_admin` (confined to accounts the row already calls administrators, so an account demoted below admin is still refused); startup rewrites a legacy `admin` role on row 1 to `root_admin` once, so the database stops disagreeing with itself; and `updateAdmin` no longer stores `admin` for row 1 — the users form submits the role it rendered even when the admin only meant to link an artist, which would demote the instance owner and, because the role changed, log them out on the way.

- **A new artist avatar now reaches the Fediverse instead of only this instance.** Uploading a profile image wrote the file, pointed the artist row at it and told nobody: `POST /api/admin/upload/avatar` has no ActivityPub access at all, and neither did the banner upload, the avatar-from-URL route, or `PUT /api/artists/:id` (name, bio, links). Remote servers cache a remote actor's `icon`, `image`, `name` and `summary` and refresh them when an actor `Update` arrives, so every follower on Mastodon or Pleroma went on seeing the old picture — the same shape as the post-edit bug, a local write with no announcement. `broadcastActorUpdate` already existed but was reachable only from the manual **Refresh federation** button in the Identity tab; all four routes now call it themselves. Announced after the response and never allowed to fail the change the artist just saved: a follower's server being unreachable is not a reason to reject an upload, so a delivery failure is logged and nothing more. Nothing is announced when the edit was refused or the artist does not exist.

- **A post edit now actually changes on Mastodon, not just on this instance.** Federating the edit as `Update` (previous entry) got it delivered; Mastodon then dropped the content change on the floor. Its `ActivityPub::Parser::StatusParser#edited_at` reads one key — the object's `updated` — and `ProcessStatusUpdateService` rewrites a status only when that timestamp is present and newer than the one it already holds; with `updated` absent it takes the implicit branch, which touches polls and interaction policies and deliberately leaves the text alone. TuneCamp's rendered `Note`/`Article` carried `published` and never `updated`, because `posts` had no column recording an edit. It has one now: `posts.updated_at`, stamped in ISO 8601 UTC (not SQLite's `CURRENT_TIMESTAMP`, which has no zone and parses ambiguously at the far end) by `updatePost`, published as `updated`, and left NULL on existing rows and on posts nobody has edited — a first publish must not announce itself as already edited. The object id is unchanged by any of this, so the `Update` still names the object remote servers already have.

- **Editing a post now reaches the Fediverse instead of stopping at the local timeline.** The edit was saved and shown on the Community tab, and nothing else heard about it: `publishPostToAP` called `broadcastPost`, which found an `ap_notes` row for that post, logged "already published … Skipping broadcast" and returned. So every follower on Mastodon or Pleroma kept reading the original text forever, and the admin's Publishing tab kept listing the post under the title it had when first published — `createApNote` was `INSERT OR IGNORE`, so re-publishing could not refresh the stored row either. An edit now federates as `Update`, the activity other servers use to replace what they cached: the object id is built from the post's slug and publish time, neither of which an edit touches, so the edited post keeps the identity remote servers already know. Adding or removing a title is the one edit that changes identity — the object turns from a `Note` into an `Article` or back, taking its id with it — and there being no way to rename an object in ActivityPub, that case retracts the old one and publishes the new. `createApNote` is now an upsert, so the Publishing list is corrected by an edit and by the Sync button alike. The Mastodon cross-post is left to the first publish: it had been firing on every edit, filing a second status for the same post, and the integration has no way to edit a status it already sent.

- **Importing a release no longer fills it with tracks that have no audio.** The Bandcamp import read the tracklist and wrote every line straight into the database as a real track, passing `localize: true` to `POST /api/tracks` on the understanding that the server would then fetch the audio and turn each one into a local file. It never did: that route destructured `localize` and used it nowhere, and `POST /api/tracks/:id/localize` — the only thing that localizes anything — handles `gdrive://` paths and refuses streaming platforms outright, which is deliberate and stays that way. So an import produced a release full of rows with `file_path: null` and a Bandcamp URL no enabled provider can play, and the artist's only way out was to delete every one of them by hand before uploading their own files. The tracklist is now staged in the editor instead of the database: titles, order, durations and a link back to the source appear as **slots**, each with its own file picker, and on save each slot that has audio is uploaded under that slot's title and position. A slot left empty is left alone — it stays listed for the next save and creates nothing. Nothing reaches the catalog until it has audio behind it, which is what the release editor was always asserting about the rows it showed.

- **Self-registration from the website no longer fails with a CORS error.** `POST /api/community/register` is documented as CORS-enabled precisely so the community website can call a directory instance straight from the visitor's browser, but `publicFederationCors` routes every mutation to `strictCors` — which emits no `Access-Control-Allow-Origin` unless the directory's own `TUNECAMP_CORS_ORIGINS` happens to list the website. The preflight came back `200`, the POST that followed was blocked by the browser, and the website reported "Could not reach any directory server" no matter how healthy the submitted instance was. That strictness protected nothing: the route carries no cookie or `Authorization` header, does nothing user-specific (it probes the submitted URL over NodeInfo and stores public metadata), and is rate-limited to one request per IP per hour — and CORS was never the CSRF boundary anyway, since a cross-site POST is sent regardless of the response headers. `POST /api/community/register` (and its preflight) now answers with wildcard CORS and no credentials; a request that does arrive with a cookie or `Authorization` header still falls through to the strict path, and every other `/api/community` mutation is unchanged.

- **A Subsonic client can see its own private playlists again.** `getPlaylists.view` called `db.getPlaylists()` with no arguments, and that defaults to a guest profile — so it answered with the public playlists only. A user's own private playlists were invisible to their Subsonic client, and an admin, who sees every playlist over REST, saw no more than anyone else. It now serves the same scope `GET /api/playlists` does: your own plus every public one, and all of them for an admin.

- **Remote cover art and avatars are served instead of silently falling back to a placeholder.** `getCoverArt.view` and `getAvatar.view` proxy an `http(s)` image by calling `response.body.pipe(res)`. Under Node's built-in `fetch` (undici) `response.body` is a WHATWG `ReadableStream`, which has no `.pipe`, so every remote image threw a `TypeError` that the surrounding `catch` swallowed — the request then fell through to the generated SVG placeholder, and the only trace was a log line. Both sites now bridge through `Readable.fromWeb`.

- **The embed's by-title album lookup was unreachable.** The fallback that resolves an album by title ran only when the specifier was `/^\d+$/`, so it was asked to match a title against a string of digits and could never fire for a real one. It now runs for the non-numeric specifiers it was written for.

- **`PlayerBar`'s rules-of-hooks suppression sat on the wrong line** — one line above the conditional `useColor` call it was meant to cover, so ESLint reported both an unused directive and the error it was supposed to silence. Also `prefer-const` on two locals in `AudioEngine.bufferToWavBlob`. The webapp lint run is back to zero errors.

- **Embed route tests no longer leak fixtures between cases.** The suite reset with `jest.clearAllMocks()`, which clears recorded calls but keeps every `mockReturnValue`, so a database lookup stubbed in one test went on answering in the next — a fixture meant to be absent came back published. Switched to `jest.resetAllMocks()`.
...
- **Lab apps, in full.** Low adoption didn't justify the maintenance and audit surface for a single-maintainer project. Gone: the `lab_apps` table and its seed rows, `/api/lab-apps` and `/api/admin/lab-apps`, the `/lab` and `/lab/:appId` webapp routes and pages, `AdminLabAppsPanel`, the `hideLab` module flag everywhere it appeared (settings store, setup wizard, admin settings, `ModuleGuard`), and `LAB.md`. [tunecamp-audiofabric](https://github.com/scobru/tunecamp-audiofabric) and [tunecamp-4-track-recorder](https://github.com/scobru/tunecamp-4-track-recorder) — built specifically for this integration — were dropped from the ecosystem entirely; [tunecamp-iris](https://github.com/scobru/tunecamp-iris) and [wormhole](https://github.com/scobru/wormhole) remain real standalone web apps, they just never embed in a TuneCamp instance again.
...
- **Single Artist portfolio mode**: New third instance mode alongside Record Label and Community. A sovereign, Bandcamp-like homepage for a solo artist, highlighting the latest release and discography with community features (Chat, Board, Network) stripped out. Selectable in the setup wizard or Admin Settings.
- **E2EE passphrase-encrypted chat rooms**: Rooms can be locked with a passphrase; the `UnlockRoomModal` lets members save/forget it locally and derives the room key via Zen SEA, so message content never leaves the client unencrypted.
