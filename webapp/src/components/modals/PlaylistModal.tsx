...
export const PlaylistModal = () => {
  ...
  // Track ID to add
  const [targetTrackId, setTargetTrackId] = useState<string | number | null>(
    null,
  );
  ...
  const loadPlaylists = async () => {
    ...
    try {
      ...
      const otherPublic = data.filter(
        (p) => p.username !== user?.username && p.isPublic,
      );
  ...
  const addToPlaylist = async (playlistId: string) => {
    ...
    try {
      ...
      const id =
        typeof targetTrackId === "string" && /^\d+$/.test(targetTrackId)
        ...
        <div className="flex-1 min-w-0">
          <div className="font-bold truncate flex items-center gap-2">
            ...
            {isMine ? (
              ...
              <span className="badge badge-xs opacity-40 font-black">
                PUBLIC
              </span>
            )}
          </div>
            ...
            >
              <div className="form-control">
                <label className="label">
                  <span className="label-text text-xs font-black tracking-normal opacity-40">
                    New Playlist Name
                  </span>
                </label>
                <input
