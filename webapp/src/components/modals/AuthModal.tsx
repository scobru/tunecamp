...
export const AuthModal = () => {
	...
	const [mode, setMode] = useState<"login" | "register" | "setup" | "forgot">(
		"login",
	);
	...
	const DEFAULT_PORTAL =
		import.meta.env.VITE_GLOBAL_PORTAL_URL || "https://www.tunecamp.org";
			...
			<div className="modal-box bg-base-100 border border-base-content/5 max-w-sm">
				...
				<h3 className="font-bold text-lg mb-6 flex items-center gap-2">
					{match(mode)
						...
						.with("setup", () => (
							<>
								<Shield size={20} /> Create First Admin
							</>
						))
						.with("forgot", () => (
							<>
				...
				{mode !== "forgot" && (
					<div
						className="tabs tabs-boxed bg-base-200 p-1 mb-6 flex flex-wrap"
						role="tablist"
					>
						<button
						...
						{showSetupOffer && (
							<div className="bg-primary/10 p-4 rounded-lg flex flex-col gap-3">
								<p className="text-sm opacity-90 text-center">
									No admin account yet. Create the first admin with the
									credentials above.
								</p>
								<button
									type="button"
									...
									<>
										...
										>
											{isFidLoading ? (
												...
												<>
													<Shield className="w-4 h-4 text-secondary" /> Sign in
													with FID
												</>
											)}
										</button>
											...
											>
												...
												<span>
													{customPortalUrl.trim()
														? `FID Portal: ${getEffectivePortalUrl()}`
														: "Configura Portale FID"}
												</span>
											</button>
