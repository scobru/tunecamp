...
export const AdminUserModal = ({
	...
	useEffect(() => {
		...
		return () =>
			window.removeEventListener("refresh-admin-artists", refreshListener);
	}, []);
	...
	useEffect(() => {
		...
		const eventListener = (e: CustomEvent) => {
			...
			if (userToEdit) {
				...
				setStorageQuota(
					userToEdit.storage_quota !== undefined ? userToEdit.storage_quota : 0,
				);
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (mode === "edit" && targetUserId) {
				// ponytail: password goes to the dedicated /password endpoint —
				// PUT /system/users/:id silently ignores a `password` field.
				if (password) {
					await API.resetUserPassword(targetUserId, password);
				}
			...
			<div className="modal-box bg-base-100 border border-base-content/5 max-h-[90vh] overflow-y-auto">
				...
				<form onSubmit={handleSubmit} className="space-y-4">
					...
					<div className="form-control">
						...
						{!isEditMode || user?.zenAuthMode !== "zen" ? (
							<input
								type="password"
								className="input input-bordered w-full"
								value={password}
								...
								<button
									...
									onClick={() => {
										const fullArtist = artists.find(
											(a) => String(a.id) === String(artistId),
										);
										dialogRef.current?.close();
										document.dispatchEvent(
											new CustomEvent("open-admin-artist-modal", {
												detail: fullArtist ?? { id: artistId },
											}),
										);
									}}
								>
							...
							<div className="flex items-start justify-between gap-4">
								<div className="flex-1 min-w-0">
									<span className="text-sm font-bold">
										Sales enabled (verified artist)
									</span>
									<p className="text-xs opacity-50 mt-1">
										Off: the linked artist publishes free content only — prices
										are stripped and checkout refuses purchases. Turn on once
										you have verified the artist owns the rights to what they
										sell.
									</p>
								</div>
								<input
					...
					<div className="form-control">
						...
						>
							...
							<option value="super_user">
								Curator (Super User / Library Management)
							</option>
					...
					<div className="form-control">
						...
						>
							...
							{![
								0, 1073741824, 5368709120, 10737418240, 21474836480,
								53687091200, 107374182400,
							].includes(Number(storageQuota)) && (
								<option value={storageQuota}>
									Custom (
									{(Number(storageQuota) / 1024 / 1024 / 1024).toFixed(1)} GB)
								</option>
							)}
						</select>
					</div>
