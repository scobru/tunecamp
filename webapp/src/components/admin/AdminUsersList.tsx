...
export const AdminUsersList = () => {
	...
	useEffect(() => {
		...
		API.getSiteSettings()
			.then((s) =>
				setSelfPublishEnabled(
					s?.listenerSelfPublish === true ||
						(s?.listenerSelfPublish as any) === "true",
				),
			)
			.catch(() => {});
			...
			<table className="table">
				...
				<tbody>
					{users.map((u) => (
						<tr
							key={u.id}
							className={clsx(selectedIds.includes(u.id) && "bg-base-200")}
						>
							{isRootAdmin && (
								<td>
									<input
							...
							<td>
								...
								{u.role === "user" && u.artist_id && (
									<span className="badge badge-accent badge-outline ml-2 text-[10px]">
										🎨 Artist
									</span>
								)}
								{u.is_active === 0 && (
								...
								{u.zen_auth_mode && (
									<span
										className={clsx(
											"badge badge-outline ml-2 text-[10px]",
											u.zen_auth_mode === "zen" && "badge-secondary",
											u.zen_auth_mode === "hybrid" && "badge-info",
											u.zen_auth_mode === "local" && "badge-ghost",
										)}
									>
										{u.zen_auth_mode === "zen"
											? "ZEN-only"
							...
							<td>
								<input
									...
									data-tip={
										u.role === "root_admin" || u.role === "admin"
											? "Admins can always use peer sharing"
											: undefined
									}
									checked={
										u.can_peer === 1 ||
									...
									onChange={async () => {
										try {
											...
											notify.success(
												`Updated peer permission for ${u.username}`,
											);
											loadUsers();
										} catch (e: any) {
