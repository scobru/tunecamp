...
export const PlayerBar = () => {
	...
	const { currentTime, progress, duration } = usePlayerStore(
		useShallow((state) => ({
	...
	const handleProgressTouchStart = (e: React.TouchEvent<HTMLDivElement>) => {
		if (!progressContainerRef.current || !duration || e.touches.length === 0)
			return;
		setIsSeeking(true);
	...
	useEffect(() => {
		...
		let newSrc = API.getStreamUrl(
			currentTrack.streamUrl || currentTrack.id,
			forceMp3 ? "mp3" : undefined,
		);
	...
	useEffect(() => {
		...
		if (audioRef.current && !crossfadingRef.current)
	...
	useEffect(() => {
		...
		const onTime = () => {
			...
			const isLosslessFormat =
				nextTrack.format &&
				["wav", "lossless"].includes(nextTrack.format.toLowerCase());
			const isLosslessExt =
				nextTrack.filename && nextTrack.filename.toLowerCase().endsWith(".wav");
			const forceMp3 =
				!nextTrack.streamUrl && (isLosslessFormat || isLosslessExt);
			let nextSrc = API.getStreamUrl(
				nextTrack.streamUrl || nextTrack.id,
				forceMp3 ? "mp3" : undefined,
			);
			...
			const startRamp = () => {
				...
				const step = () => {
					...
					try {
						deckB.volume = Math.max(0, targetVol * (1 - t));
					} catch {
						/* ignore */
					}
					if (audioRef.current)
						audioRef.current.volume = Math.min(targetVol, targetVol * t);
					if (t < 1) {
	...
	let coverUrl = currentTrack
		? currentTrack.coverImage ||
			...
			(currentTrack.albumId
				? API.getAlbumCoverUrl(currentTrack.albumId, cacheBuster)
				: "") ||
			(currentTrack.id
				? API.getTrackCoverUrl(currentTrack.id, cacheBuster)
				: "") ||
			(currentTrack.artistId
				? API.getArtistCoverUrl(currentTrack.artistId, cacheBuster)
				: "")
		: "";
	...
	const isAdminOrOwner =
		...
		(currentTrack.owner_id &&
			String(currentTrack.owner_id) ===
				String(useAuthStore.getState().user?.id)) ||
		(currentTrack.artistId &&
			String(currentTrack.artistId) ===
				String(useAuthStore.getState().user?.artistId));
	...
	const handleStarArtist = async () => {
		...
		try {
			const id = currentTrack.artistId
				? String(currentTrack.artistId)
				: `ext:artist:${currentTrack.artistName}`;
	...
	const handleStarAlbum = async () => {
		...
		try {
			const id = currentTrack.albumId
				? String(currentTrack.albumId)
				: `ext:album:${currentTrack.album_title}`;
			await API.starAlbum(id, {
				title: currentTrack.album_title,
				...
				<div className="flex items-center gap-4 w-1/2 lg:w-1/4 min-w-0">
					...
					<div className="min-w-0">
						{albumDetailTarget ? (
							...
							<h3 className="font-black text-title-small lg:text-title-medium truncate tracking-tight">
								{currentTrack.title}
							</h3>
						)}
						{currentTrack.artistId ? (
							...
							<p className="text-xs lg:text-label-small font-medium opacity-60 text-primary truncate tracking-normal">
								{currentTrack.artistName}
							</p>
						)}
					</div>
				...
				<div className="flex flex-col items-center flex-1 max-w-xl gap-1">
					<div className="flex items-center gap-3 lg:gap-6">
						<button
							...
							className={clsx(
								"hidden lg:flex btn btn-ghost btn-xs btn-circle transition-all tooltip tooltip-top",
								isShuffled
						...
						>
							{isPlaying ? (
								...
								<Play
									className="w-5 h-5 lg:w-7 lg:h-7 ml-1"
									fill="currentColor"
									aria-hidden="true"
								/>
							)}
						...
						<button
							...
							className={clsx(
								"hidden lg:flex btn btn-ghost btn-xs btn-circle relative transition-all tooltip tooltip-top",
								repeatMode !== "none"
									? "text-primary scale-110"
									: "opacity-40 hover:opacity-100",
							)}
						...
						>
							...
							{repeatMode === "one" && (
								<span className="absolute -top-1 -right-1 text-[8px] font-black bg-primary text-primary-content rounded-full w-3 h-3 flex items-center justify-center">
									1
								</span>
							)}
						</button>
					...
					<div className="w-full flex items-center gap-4 text-[11px] lg:text-xs font-black tracking-normal opacity-40 h-6">
						<span className="w-8 lg:w-10 text-right tabular-nums">
							{formatDuration(
								isSeeking ? (localProgress / 100) * duration : currentTime,
							)}
						</span>
				...
				<div className="flex items-center gap-2 lg:gap-6 w-auto lg:w-1/4 justify-end">
					<div className="hidden lg:flex items-center gap-3">
						<Volume2
							...
							className={clsx(
								"opacity-40",
								volume === 0 && "text-error opacity-100",
							)}
						/>
						<input
							aria-label="Volume"
					...
					<div className="flex gap-1 h-10 items-center border-l border-base-content/5 pl-2 lg:pl-4 ml-1 lg:ml-2">
						<div className="dropdown dropdown-top dropdown-end">
							...
							<ul
								tabIndex={0}
								className="dropdown-content z-[60] menu p-2 shadow-level-1 bg-base-300 rounded-2xl w-56 border border-base-content/10 mb-4"
							>
								<li className="lg:hidden">
									<a onClick={toggleShuffle}>
										<Shuffle
											size={16}
											className={clsx(isShuffled && "text-primary")}
										/>{" "}
										Shuffle
									</a>
								</li>
								<li className="lg:hidden">
									<a onClick={toggleRepeat}>
										<Repeat
											size={16}
											className={clsx(repeatMode !== "none" && "text-primary")}
										/>{" "}
										Repeat: {repeatMode}
									</a>
								</li>
								...
								<li>
									<a onClick={toggleLyrics}>
										<Mic2
											size={16}
											className={clsx(isLyricsOpen && "text-primary")}
										/>{" "}
										Lyrics
									</a>
								</li>
								<li>
									<a onClick={toggleCanvas}>
										<Maximize2
											size={16}
											className={clsx(isCanvasOpen && "text-primary")}
										/>{" "}
										Visualizer
									</a>
								</li>
								<li>
									<a onClick={toggleRadio}>
										<Radio
											size={16}
											className={clsx(isRadioMode && "text-primary")}
										/>{" "}
										Radio Mode
									</a>
								</li>
								...
								<li className="menu-title flex-row items-center gap-2 px-2 py-1 text-[11px]">
									<AudioLines
										size={14}
										className={clsx(crossfadeSec > 0 && "text-primary")}
									/>{" "}
									Crossfade
								</li>
								<li>
								...
								<div className="divider my-1 opacity-10"></div>
								<li>
									<a onClick={handleAddToPlaylist}>
										<ListMusic size={16} /> Add to Playlist
									</a>
								</li>
								<li>
									<a onClick={handleStarArtist}>
										<User size={16} /> Favorite Artist
									</a>
								</li>
								<li>
									<a onClick={handleStarAlbum}>
										<Disc size={16} /> Favorite Album
									</a>
								</li>
								{isAdminOrOwner && (
									<>
						...
						<button
							...
							className={clsx(
								"btn btn-ghost btn-sm btn-square opacity-40 hover:opacity-100 tooltip tooltip-top",
							)}
							onClick={toggleQueue}
							data-tip="Play Queue"
						>
