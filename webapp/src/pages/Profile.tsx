...
import { SubsonicPasswordCard } from "../components/ui/SubsonicPasswordCard";
...
const Profile = () => {
	const { user, isAuthenticated, role, isInitializing, checkAuth } =
		useAuthStore();
	...
	const [artistRequestedAt, setArtistRequestedAt] = useState<string | null>(
		null,
	);
	...
	const handleDeleteToken = async (id: number) => {
		if (
			!(await confirm(
				"Are you sure you want to revoke this API token? Any applications using it will lose access.",
			))
		)
			return;
		try {
	...
	const handleRequestArtist = async () => {
		...
		try {
			...
			if (result?.autoApproved && result?.token) {
				...
				await checkAuth();
				notify.success(
					"Artist profile created! You can now publish your music.",
				);
			} else {
				...
				{activeTab === "settings" && (
					<div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-300">
						<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
							<div className="card bg-base-200 border border-base-content/10 p-6 space-y-6">
								...
								<div className="form-control w-full">
									...
									<label className="label">
										<span className="label-text-alt opacity-40 italic">
											{emailMessage ||
												"Used to send you a password reset link."}
										</span>
									</label>
								</div>
								...
								<div className="alert alert-info bg-primary/10 border-primary/20 text-sm">
									<Settings size={18} />
									<span>Profile changes are saved on this server.</span>
								</div>
							</div>

							<div className="card bg-base-200 border border-base-content/10 p-6 space-y-6">
								<h3 className="text-xl font-bold flex items-center gap-2">
									<Camera size={20} className="text-secondary" /> Profile
									Visuals
								</h3>
								<p className="text-sm opacity-60">
									Your profile picture is saved on the server. Square images
									work best (PNG/JPG, max 400px).
								</p>
							...
							{/* Subsonic apps can't use the account password (FID accounts
							    don't have one at all), so every account gets this. */}
							<SubsonicPasswordCard />
							...
							<div className="card bg-base-200 border border-base-content/10 p-6 space-y-4">
								...
								<p className="text-sm opacity-60">
									Let anyone view a public page with your name, avatar, public
									playlists, and your likes on public releases. Your collection,
									purchases, wallet, and settings are never shown. Off by
									default.
								</p>
								<label className="flex items-center gap-3 cursor-pointer">
									<input
										...
										onChange={(e) =>
											handleTogglePublicProfile(e.target.checked)
										}
									/>
							...
							{!hasArtistProfile && (
								<div className="card bg-base-200 border border-base-content/10 p-6 space-y-4">
									<h3 className="text-xl font-bold flex items-center gap-2">
										<User size={20} className="text-secondary" /> Become an
										Artist
									</h3>
									<p className="text-sm opacity-60">
										{selfPublishEnabled
									...
									{artistRequestedAt ? (
										<div className="alert alert-info bg-primary/10 border-primary/20 text-sm">
											...
											<span>
												Request sent on{" "}
												{new Date(artistRequestedAt).toLocaleDateString()}.
							...
							<div className="card bg-base-200 border border-base-content/10 p-6 space-y-6 md:col-span-2">
								<h3 className="text-xl font-bold flex items-center gap-2">
									<Shield size={20} className="text-secondary" /> Account Role &
									Permissions
								</h3>
								<p className="text-sm opacity-60">
									Here you can view your current role within the TuneCamp
									platform and see what actions you can perform.
								</p>
								...
								<div className="bg-base-300/40 rounded-2xl p-6 border border-base-content/10 flex flex-col md:flex-row gap-6 items-start">
									<div className="flex flex-col gap-2 min-w-[200px]">
										<span className="text-xs opacity-50 tracking-normal font-black">
											Current Role
										</span>
										<div
											className={clsx(
												"badge badge-lg gap-2 font-bold py-4 px-5 text-sm tracking-normal rounded-xl shadow-sm border-0",
												role === "root_admin"
									...
									<div className="flex-1 space-y-4">
										<div>
											...
											<p className="text-sm opacity-70 mt-1">
												{role === "root_admin" &&
													"You have complete administrative and security control over this TuneCamp instance."}
												{role === "admin" &&
													"You have administrative privileges to moderate the community, manage releases, and support artists."}
												{role === "super_user" &&
													"You are responsible for music catalog quality, organization, and correcting track metadata."}
												{role === "user" &&
													(user?.artistId
										...
										<div className="space-y-3">
											<span className="text-xs opacity-50 tracking-normal font-bold">
												What you can do:
											</span>
											<ul className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
												{role === "root_admin" &&
													[
														...
														<li
															key={idx}
															className="flex gap-2 items-start text-base-content/80"
														>
															<Check
																size={16}
												...
												{role === "user" &&
													[
														...
														...(user?.artistId
															? [
																	"Publish your own music and manage your releases",
																	"Connect Stripe to sell your music",
																]
															: [
						...
						{isCuratorOrAbove && (
							<div className="card bg-base-200 border border-base-content/10 p-6 space-y-4">
								...
								<p className="text-sm opacity-60">
									Create personal API tokens to programmatically interact with
									the TuneCamp API. These tokens will run with your role's
									permissions (
									{role === "root_admin"
										? "Root Admin"
										: role === "admin"
										...
										<table className="table w-full text-sm">
											<thead>
												<tr>
													...
													<th className="bg-transparent opacity-60">
														Created At
													</th>
													<th className="bg-transparent opacity-60 w-16 text-center">
														Actions
													</th>
												</tr>
											</thead>
						...
						{purchasesLoading || loadingTracks ? (
							...
							<div className="p-20 text-center opacity-40 bg-base-200/50 rounded-3xl border border-dashed border-base-content/20">
								...
								<p className="text-sm mt-2">
									Purchase tracks or redeem an unlock code to build your
									collection.
								</p>
							</div>
						) : (
					...
					<div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
						{artistLoading ? (
							...
							<ArtistProfileEditor
								...
								onSaved={(data) => setArtistData(data)}
							/>
						)}
					</div>
				)}
...
const ArtistProfileEditor = ({
		...
		if (initialData) {
			...
			if (initialData.links) {
				...
				const donation = links.find(
					(l: any) =>
						l.type === "support" || l.platform?.toLowerCase() === "donation",
				);
			...
			if (initialData.postParams) {
				const params = initialData.postParams as {
					instance?: string;
					token?: string;
				};
	...
	const handleUpdateAliases = async (updatedAliases: string[]) => {
		...
		try {
			await API.updateArtistAlias(
				initialData.id,
				updatedAliases.length > 0 ? updatedAliases : null,
			);
	...
	const handleInitiateMove = async () => {
		...
		if (
			!(await confirm(
				`Are you sure you want to initiate migration of this identity to ${targetMoveUri}?\nThis will broadcast a Move activity to all followers and they will automatically follow the new profile.`,
			))
		)
			return;
		setIsMoving(true);
		try {
			...
			notify.success(
				"Move initiated successfully! Followers will be redirected to your new profile.",
			);
		} catch (err: any) {
	...
	const handleImportIdentity = async () => {
		...
		try {
			...
			notify.success(
				"Profile imported and verified successfully! Refreshing details...",
			);
			window.location.reload();
		} catch (err: any) {
	...
	const handleSync = async () => {
		...
		if (
			!(await confirm(
				"Synchronize your releases and posts with the Fediverse?",
			))
		)
			return;
		setIsSyncing(true);
			...
			<div className="lg:col-span-4 space-y-6">
				<div className="card bg-base-200 border border-base-content/10 p-6 space-y-6">
					...
					<div className="flex flex-col items-center gap-4 py-2">
						...
						<p className="text-xs opacity-40">
							Square images work best (PNG/JPG)
						</p>
					</div>
					...
					<div className="form-control">
						...
						<label className="label">
							<span className="label-text-alt opacity-40 italic">
								Display name only — URL slug (<code>/{initialData?.slug}</code>)
								does not change.
							</span>
						</label>
					</div>
				...
				<div className="card bg-base-200 border border-base-content/10 p-6 space-y-4">
					...
					<p className="text-xs opacity-50">
						Custom background image shown at the top of your artist page.
					</p>
					<div className="relative w-full h-28 rounded-xl overflow-hidden bg-neutral border border-base-content/10 shadow-level-1 group cursor-pointer">
						{bannerPreview ? (
							...
							<img
								src={API.getArtistBannerUrl(initialData.id, Date.now())}
								className="w-full h-full object-cover"
							/>
						) : (
					...
					<p className="text-xs opacity-40">
						Wide images work best (16:9 or panoramic). PNG/JPG.
					</p>
				</div>

				{/* Status Section */}
				<div className="card bg-primary/5 border border-primary/10 p-6 space-y-4">
					<h3 className="text-sm font-bold tracking-normal opacity-40">
						Digital Presence
					</h3>
			...
			<div className="lg:col-span-8 space-y-8">
				<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
					...
					<div className="card bg-base-200 border border-base-content/10 p-6 space-y-4">
						...
						<div className="form-control">
							<label className="label">
								<span className="label-text font-bold text-emerald-400">
									Wallet Address
								</span>
							</label>
							<input
							...
							<label className="label">
								<span className="label-text-alt opacity-40">
									Direct-to-Artist revenue goes here.
								</span>
							</label>
						</div>
				...
				<div className="card bg-base-200 border border-base-content/10 overflow-hidden">
					<div className="bg-base-300/50 p-6 border-b border-base-content/10">
						<h3 className="text-xl font-bold flex items-center gap-2">
							<Play size={20} className="text-indigo-400" /> Federation &
							Automation
						</h3>
						<p className="text-xs opacity-50 mt-1">
							Cross-post activities and manage community outreach.
						</p>
					</div>
				...
				<div className="card bg-base-200 border border-base-content/10 overflow-hidden">
					<div className="bg-base-300/50 p-6 border-b border-base-content/10">
						<h3 className="text-xl font-bold flex items-center gap-2">
							<Globe size={20} className="text-violet-400" /> Identity Migration
							(ActivityPub)
						</h3>
						<p className="text-xs opacity-50 mt-1">
							Link your remote Fediverse profiles, import metadata, or migrate
							your followers.
						</p>
					</div>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
							<div>
								<h4 className="text-sm font-bold opacity-70 mb-3">
									ActivityPub Addresses
								</h4>
								<div className="space-y-3">
									<div className="form-control">
										<span className="label-text text-xs opacity-50 mb-1">
											Local Handle
										</span>
										<div className="flex gap-2">
											<input
											...
											<button
												...
												onClick={() => {
													navigator.clipboard.writeText(
														`@${initialData?.slug}@${window.location.host}`,
													);
									...
									<div className="form-control">
										...
										<div className="flex gap-2">
											...
											<button
												...
												onClick={() => {
													navigator.clipboard.writeText(
														`${window.location.origin}/users/${initialData?.slug}`,
													);
								...
								<div className="space-y-3">
									{aliases.length === 0 ? (
										<p className="text-xs opacity-40 italic">
											No aliases verified yet. Add aliases to prove ownership of
											remote accounts.
										</p>
									) : (
										<div className="flex flex-col gap-2 max-h-36 overflow-y-auto">
											{aliases.map((aliasUri, idx) => (
												<div
													key={idx}
													className="flex items-center justify-between gap-2 bg-base-300/50 p-2 rounded-lg border border-base-content/10"
												>
													<span
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
							<div className="bg-base-300/40 p-4 rounded-xl border border-base-content/10 space-y-3">
								...
								<p className="text-xs opacity-60">
									Copy profile metadata (bio, links, etc.) from a remote
									Fediverse account to this instance.
								</p>
								<div className="alert alert-info bg-info/5 border-info/20 text-xs py-2 px-3">
									<span>
										<strong>Requirement:</strong> The remote account must first
										add your local Actor URI to their aliases.
									</span>
								</div>
								<div className="flex gap-2">
									<input
							...
							<div className="bg-base-300/40 p-4 rounded-xl border border-base-content/10 space-y-3">
								...
								<p className="text-xs opacity-60">
									Migrate your followers to another ActivityPub profile. This
									will redirect followers to your new instance.
								</p>
								{initialData?.moved_to ? (
									<div className="alert alert-warning bg-warning/10 border-warning/20 text-xs py-2 px-3">
										<span>
											...
											<strong>{initialData.moved_to}</strong>
										</span>
									</div>
								) : (
