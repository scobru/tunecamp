...
// Shared db-client stub the mocked Discogs Client will return from .database()
const dbClient = {
    search: jest.fn(),
    getRelease: jest.fn()
};

jest.unstable_mockModule('disconnect', () => ({
    default: {
        Client: class {
            // matches `new DiscogsClient(userAgent, opts)`
            constructor(_userAgent: string, _opts?: any) {}
            database() { return dbClient; }
        }
    }
}));

const { DiscogsProvider } = await import('../discogs.provider.js');
...
describe('DiscogsProvider', () => {
    ...
    describe('searchRelease', () => {
        test('maps results, splitting "Artist - Title" titles', async () => {
            (dbClient.search as any).mockImplementationOnce((_params: any, cb: any) =>
                cb(null, {
                    results: [{
                        id: 42,
        ...
        test('handles an empty/absent results array', async () => {
            (dbClient.search as any).mockImplementationOnce((_params: any, cb: any) => cb(null, {}));
            expect(await provider.searchRelease('q')).toEqual([]);
        });
    });
    ...
    describe('searchRecording', () => {
        test('delegates to searchRelease', async () => {
            ...
            expect(r.id).toBe('1');
            expect(dbClient.search).toHaveBeenCalledWith(
    ...
    describe('getCoverUrl', () => {
        test('returns the first image resource url', async () => {
            (dbClient.getRelease as any).mockImplementationOnce((_id: any, cb: any) =>
        ...
        test('returns null when there are no images', async () => {
            (dbClient.getRelease as any).mockImplementationOnce((_id: any, cb: any) => cb(null, {}));
