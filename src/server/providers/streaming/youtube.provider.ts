...
export class YouTubeStreamingProvider implements StreamingProvider, MetadataProvider {
    ...
    public async reset(newPath?: string) {
        // Search for cookies in multiple locations common in Docker/Local setups
        const possiblePaths = [
            newPath,
        ...
        const finalPath = possiblePaths[0];
        
        if (finalPath) {
            this.cookiesPath = finalPath;
            console.log(`[YouTubeProvider] 🔑 Cookies found and set to: ${this.cookiesPath}`);
            ...
            console.log(`[YouTubeProvider] 🔑 No cookies found in search paths. Local resolution might be limited.`);
        }
    ...
    private async _resolveStreamUrl(urlOrId: string): Promise<string | null> {
        ...
        // --- Cobalt API Fallback (Modern alternative to "notube") ---
        const cobaltUrl = await this._resolveViaCobalt(targetUrl);
    ...
    private async _resolveViaCobalt(targetUrl: string): Promise<string | null> {
        console.log(`[YouTubeProvider] 📡 Attempting Cobalt API fallback for ${targetUrl}...`);
        const cobaltInstances = [
            "https://api.cobalt.tools",
            "https://cobalt.api.unv.cc",
            "https://cobalt.sh",
            "https://cobalt.populas.org"
        ];
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (data.status === 'stream' && data.url) {
                        console.log(`[YouTubeProvider] ✨ Success! Resolved via Cobalt instance: ${instance}`);
                        return data.url;
                    }
                }
            } catch (e) {
