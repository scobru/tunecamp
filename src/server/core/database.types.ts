...
import type { VisibilityProfile, ViewerContext } from "../common/visibility.js";
...
export interface IdentityManager {
	...
	getPluginState(
		id: string,
	): { enabled: boolean; config: string | null } | undefined;
...
export interface LibraryManager {
	...
	getReleasesByArtist(
		artistId: number,
		profile?: VisibilityProfile | ViewerContext,
		artistName?: string,
	): Release[];
	getReleasesByOwner(
		ownerId: number,
	...
	createRelease(
		release: Omit<Release, "id" | "created_at" | "artist_name" | "artist_slug">,
	): number;
	...
	getAlbumsWithStats(
		profile?: VisibilityProfile | ViewerContext,
	): (Album & { songCount: number; duration: number })[];
	...
	createAlbum(
		album: Omit<Album, "id" | "created_at" | "artist_name" | "artist_slug">,
	): number;
	...
	getTrackExternalArtworkByAlbumId(
		albumId: number,
		profile?: VisibilityProfile | ViewerContext,
	): string | null;
	getTracksByArtist(
	...
	createTracks(
		tracks: Omit<Track, "id" | "album_title" | "created_at">[],
	): number[];
	...
	getTrackPriceFromRelease(
		...
		| { price: number; price_usdc: number; currency: string; title: string }
		| undefined;
	...
	getAlbumsMissingMetadata(
		filter: "genre" | "year" | "cover" | "description" | "artist",
	): Album[];
...
export interface SocialManager {
	...
	removeAllFollowers(actorUri: string): void; // Drops actor from all artists
	updateFollowerUri(
		oldActorUri: string,
		newActorUri: string,
		newInboxUri: string,
		newSharedInboxUri?: string,
	): void;
	...
	starItems(
		user: string,
		items: { type: "track" | "album" | "artist"; id: string }[],
	): void;
	unstarItems(
		user: string,
		items: { type: "track" | "album" | "artist"; id: string }[],
...
export interface IntegrationManager {
	...
	getStorageAccountByProvider(
		userId: number,
		provider: string,
	): StorageAccount | undefined;
	createStorageAccount(
		account: Omit<StorageAccount, "id" | "created_at">,
	): number;
...
export interface PeerManager {
	...
	replacePeerTracks(
		sessionId: string,
		tracks: Omit<PeerTrack, "session_id" | "created_at">[],
	): void;
