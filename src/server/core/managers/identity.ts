...
export function createIdentityManager(db: DatabaseType): IdentityManager {
	return {
		// Users
		getUser: (id: number) =>
			db.prepare("SELECT * FROM admin WHERE id = ?").get(id) as
				| User
				| undefined,
		getUserByUsername: (u: string) =>
			db.prepare("SELECT * FROM admin WHERE username = ?").get(u) as
				| User
				| undefined,
		getUserByArtistId: (aid: number) =>
			db.prepare("SELECT * FROM admin WHERE artist_id = ?").get(aid) as
				| User
				| undefined,
		createUser(u: string, p: string, aid?: number | null, r = "admin"): number {
			return Number(
				db
					.prepare(
						"INSERT INTO admin (username, password_hash, artist_id, role) VALUES (?, ?, ?, ?)",
					)
					.run(u, p, aid || null, r).lastInsertRowid,
			);
		},
		...
		getAdmins: () =>
			db
				.prepare(
					"SELECT * FROM admin WHERE role IN ('admin', 'super_user', 'root_admin')",
				)
				.all() as User[],
		updateSubscription(
			userId: number,
			...
			db.prepare(
				"UPDATE admin SET subscription_status = ?, subscription_expires_at = ? WHERE id = ?",
		...
		getUserSubscription(userId: number): {
			...
			const row = db
				.prepare(
					"SELECT subscription_status, subscription_expires_at FROM admin WHERE id = ?",
				)
				.get(userId) as any;
			return {
		...
		// Settings
		getSetting: (k: string) =>
			(db.prepare("SELECT value FROM settings WHERE key = ?").get(k) as any)
				?.value,
		...
		getPluginState: (id: string) => {
			const r = db
				.prepare("SELECT enabled, config FROM system_plugins WHERE id = ?")
				.get(id) as any;
			return r ? { enabled: !!r.enabled, config: r.config } : undefined;
		},
		setPluginEnabled: (id: string, e: boolean) => {
			db.prepare(
				"INSERT OR REPLACE INTO system_plugins (id, enabled) VALUES (?, ?)",
			).run(id, e ? 1 : 0);
		},
		setPluginConfig(id: string, config: string) {
			db.prepare("UPDATE system_plugins SET config = ? WHERE id = ?").run(
				config,
				id,
			);
		},
