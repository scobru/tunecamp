...
export async function loadConfig(
	...
	if (!jwtSecret) {
		...
		if (jwtSecret && !secretFileExists && legacySecretExists) {
			...
			try {
				fs.promises
					.writeFile(secretFilePath, jwtSecret)
					.then(() =>
						console.log(
							`🔒 Migrated JWT secret to stable location: ${secretFilePath}`,
						),
					)
			...
			try {
				fs.promises
					...
					.then(() =>
						fs.promises.writeFile(secretFilePath, jwtSecret as string, {
							mode: 0o600,
						}),
					)
					.then(() =>
						console.log(
							`🔒 Generated new JWT secret and saved to ${secretFilePath} (restricted permissions)`,
						),
					)
					.catch((err) =>
						console.warn(
							"⚠️  Could not save JWT secret to file, sessions may be lost on restart:",
							err,
						),
					);
			} catch (err) {
	...
	const finalMusicDir =
		overrides?.musicDir || process.env.TUNECAMP_MUSIC_DIR || defaultMusicDir;
	...
	const defaultTranscodeCacheDir = path.join(
		path.dirname(finalDbPath),
		"cache",
		"transcode",
	);
	const transcodeCacheDir =
		process.env.TUNECAMP_TRANSCODE_CACHE_DIR ||
		overrides?.transcodeCacheDir ||
		defaultTranscodeCacheDir;
	const transcodeCacheMaxBytes =
		parseInt(process.env.TUNECAMP_TRANSCODE_CACHE_MAX_BYTES || "", 10) ||
		overrides?.transcodeCacheMaxBytes ||
	...
	return {
		...
		federationSeeds:
			process.env.TUNECAMP_FEDERATION_SEEDS?.split(/[,\s]+/)
				.map((p) => p.trim())
				.filter((p) => p.length > 0) || overrides?.federationSeeds,
		adminUser:
			process.env.TUNECAMP_ADMIN_USER || overrides?.adminUser || "admin",
		adminPass:
			process.env.TUNECAMP_ADMIN_PASS || overrides?.adminPass || "admin",
		downloadDir:
			process.env.TUNECAMP_DOWNLOAD_DIR ||
			overrides?.downloadDir ||
			defaultDownloadDir,
		stripeOnrampSecretKey:
			process.env.STRIPE_ONRAMP_SECRET_KEY ||
			process.env.STRIPE_SECRET_KEY ||
			overrides?.stripeOnrampSecretKey,
		telegramBotToken:
			process.env.TUNECAMP_TELEGRAM_BOT_TOKEN ||
			process.env.TELEGRAM_BOT_TOKEN ||
			overrides?.telegramBotToken,
		telegramMasterId:
			process.env.TUNECAMP_TELEGRAM_MASTER_ID ||
			process.env.TELEGRAM_MASTER_ID ||
			overrides?.telegramMasterId,
		openrouterApiKey:
		...
		openrouterModel:
			...
			"openrouter/free",
		stripeSecretKey:
			process.env.STRIPE_SECRET_KEY || overrides?.stripeSecretKey,
		stripeWebhookSecret:
			process.env.STRIPE_WEBHOOK_SECRET || overrides?.stripeWebhookSecret,
		gdriveClientId:
			process.env.TUNECAMP_GDRIVE_CLIENT_ID || overrides?.gdriveClientId,
		gdriveClientSecret:
			process.env.TUNECAMP_GDRIVE_CLIENT_SECRET ||
			overrides?.gdriveClientSecret,
		brevoApiKey: process.env.BREVO_API_KEY || overrides?.brevoApiKey,
		brevoSenderEmail:
			process.env.BREVO_SENDER_EMAIL || overrides?.brevoSenderEmail,
		brevoSenderName:
			process.env.BREVO_SENDER_NAME || overrides?.brevoSenderName,
		liveEnabled:
		...
		xaccelRedirect:
			process.env.TUNECAMP_XACCEL_REDIRECT === "true" ||
			overrides?.xaccelRedirect ||
			false,
		xaccelMediaPrefix:
			process.env.TUNECAMP_XACCEL_MEDIA_PREFIX ||
			overrides?.xaccelMediaPrefix ||
			"/_protected_media",
		xaccelCachePrefix:
...
export async function readJwtSecret(
	...
	if (secretFileExists) {
		...
		try {
			await fs.promises.access(legacySecretPath);
			jwtSecret = (
				await fs.promises.readFile(legacySecretPath, "utf-8")
			).trim();
		} catch {
			// expected: file may not exist
		}
