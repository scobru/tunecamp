...
export function createAuthService(db: any, jwtSecret: string, adminUser = "admin", adminPass = "admin") {
        ...
        if (!tableExists) {
            ...
            if (!hasUsername || !hasArtistId || !hasRole || !hasGunPub || !hasSubsonic || !hasIsActive) {
                ...
                for (const old of oldAdmins) {
                    ...
                    if (!hasUsername && old.id === 1)
                        username = 'admin';
                    insertStmt.run(old.id, username, old.password_hash, old.created_at, old.updated_at, old.artist_id || null, old.gun_pub || null, old.gun_priv || null, old.subsonic_token || null, old.subsonic_password || null, old.is_active !== undefined ? old.is_active : 1, old.token_version || 0);
                }
                // 4. Drop old table
    ...
    return {
        async init() {
            const user = db.prepare("SELECT id, password_hash, role FROM admin WHERE username = ?").get(adminUser);
            if (!user) {
            ...
            const count = db.prepare("SELECT COUNT(*) as count FROM admin").get().count;
            if (count === 0) {
                console.log("🆕 First run detected: No users found in database.");
            }
        },
        async isDefaultPassword(username: string) {
            const user = db.prepare("SELECT password_hash FROM admin WHERE username = ?").get(username);
            if (!user)
                return false;
        ...
        async verifyToken(token: string) {
            try {
                ...
                const user = db.prepare("SELECT token_version, is_active FROM admin WHERE id = ?").get(decoded.userId);
        ...
        async authenticateUser(username: string, password?: string, pubKey?: string, proof?: any) {
            console.log(`[AUTH] Attempting login for user: '${username}' (password provided: ${!!password}, pubKey: ${!!pubKey})`);
            let user = db.prepare("SELECT id, username, password_hash, artist_id, role, gun_pub, gun_priv, is_active, token_version FROM admin WHERE username = ?").get(username);
            if (!user) {
                // Try case-insensitive fallback if not found (just in case)
                user = db.prepare("SELECT id, username, password_hash, artist_id, role, gun_pub, gun_priv, is_active, token_version FROM admin WHERE username = ? COLLATE NOCASE").get(username);
                if (user)
                    console.log(`[AUTH] Found user '${user.username}' via case-insensitive lookup for '${username}'`);
            }
            ...
            if (pubKey && proof) {
                ...
                if (isValid) {
                    ...
                    if (!user) {
                        ...
                        const { id } = await this.createUser(username, tempPass, null); // artist creation below
                        // Link the pubKey
                        db.prepare("UPDATE admin SET gun_pub = ? WHERE id = ?").run(pubKey, id);
                        // Reload user record
                        user = db.prepare("SELECT id, username, password_hash, artist_id, role, gun_pub, gun_priv, is_active FROM admin WHERE id = ?").get(id);
                    }
            ...
            let userRole = user.role ? user.role : UserRole.ADMIN;
            if (user.id === 1)
                userRole = UserRole.ROOT_ADMIN;
            // Handle ZEN Key Management for all users
            let gunPair = undefined;
            if (user.gun_pub && user.gun_priv) {
                try {
            ...
            if (user.id === 1) {
                try {
                    const systemPairRow = db.prepare("SELECT value FROM settings WHERE key = 'gunPair'").get();
                    if (systemPairRow) {
            ...
            if (!artistId) {
                ...
                let existingArtist = db.prepare("SELECT id FROM artists WHERE name = ? COLLATE NOCASE").get(username);
        ...
        async verifyZenSignature(message: string, pubKey: string, proof: any) {
            try {
                if (!proof || !pubKey)
                    return false;
                ...
                const verified = await Zen.verify(parsedProof, pubKey);
        ...
        async createAdmin(username: string, password: string, artistId: number | null = null, role: string = UserRole.ADMIN) {
        ...
        async createUser(username: string, password: string, artistId: number | null, storageQuota = 1024 * 1024 * 1024, pubKey?: string, role: string = UserRole.NORMAL_USER) {
            const hash = await this.hashPassword(password);
        ...
        updateAdmin(id: number, artistId: number | null, role?: string) {
            if (role) {
                if (id === 1 && role !== 'admin') {
        ...
        updateStorageUsed(userId: number, bytesUsed: number) {
        ...
        getStorageInfo(userId: number) {
            return db.prepare("SELECT storage_quota, storage_used FROM admin WHERE id = ?").get(userId);
        },
        ...
        deleteAdmin(id: number) {
            ...
            const adminCount = db.prepare("SELECT COUNT(*) as count FROM admin WHERE role = 'admin'").get().count;
            const user = db.prepare("SELECT role FROM admin WHERE id = ?").get(id);
        ...
        async changePassword(username: string, newPassword: string) {
            ...
            const user = db.prepare("SELECT id, gun_pub FROM admin WHERE username = ?").get(username);
            if (user && !user.gun_pub) {
                console.log(`🔐 [AUTH] Generating new ZEN Identity for ${username} during password change...`);
                const gunPair = await Zen.pair();
        ...
        isRootAdmin(username: string) {
            const row = db.prepare("SELECT id FROM admin WHERE username = ?").get(username);
            return row?.id === 1;
        },
        getUserPair(username: string) {
            const user = db.prepare("SELECT gun_priv FROM admin WHERE username = ?").get(username);
            if (!user || !user.gun_priv)
                return null;
            try {
        ...
        updateZenPair(username: string, pair: any) {
        ...
        // Mastodon
        async registerMastodonApp(instanceUrl: string, redirectUri: string) {
            // Cleanup URL
            ...
            const existing = db.prepare("SELECT * FROM oauth_clients WHERE instance_url = ?").get(baseUrl);
            if (existing) {
                return {
        ...
        getMastodonAuthUrl(instanceUrl: string, clientId: string, redirectUri: string) {
            const url = new URL(instanceUrl.startsWith("http") ? instanceUrl : `https://${instanceUrl}`);
        ...
        async exchangeMastodonCode(instanceUrl: string, clientId: string, clientSecret: string, redirectUri: string, code: string) {
            const url = new URL(instanceUrl.startsWith("http") ? instanceUrl : `https://${instanceUrl}`);
            // Validate SSRF
        ...
        async loginWithMastodon(instanceUrl: string, redirectUri: string, code: string) {
            // 1. Get Client
            ...
            const link = db.prepare("SELECT * FROM oauth_links WHERE provider = ? AND subject = ?").get(provider, subject);
            if (link) {
                // Decrypt and return
        ...
        // Encryption helpers
        encryptZenPriv(priv: any) {
            return encryptZenPrivHelper(priv, jwtSecret);
        },
        decryptZenPriv(encrypted: string) {
            return decryptZenPrivHelper(encrypted, jwtSecret);
        },
        async deriveZenWallet(pair: any, id?: string) {
            if (!pair || !pair.priv) {
 ...
 */
export function encryptZenPrivHelper(priv: any, secret: string) {
 ...
 */
export function decryptZenPrivHelper(encrypted: string, secret: string) {
