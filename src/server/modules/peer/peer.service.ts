...
export class PeerService {
	...
	startHeartbeat() {
		this.pingInterval = setInterval(() => {
			...
			try {
				...
				console.error(
					"🔌 [PeerService] Failed to purge stale peer sessions:",
					e,
				);
			}
			...
			for (const [sessionId, session] of this.activeSessions.entries()) {
				...
				if (session.ws.readyState !== 1) {
					// 1 = OPEN
					console.log(
						`🔌 [PeerService] Session ${sessionId} closed or broken. Cleaning up.`,
					);
				...
				if (now - session.lastPongAt > 60000) {
					console.log(
						`🔌 [PeerService] Session ${sessionId} missed heartbeat. Cleaning up.`,
					);
					this.unregisterSession(sessionId);
					continue;
				}
				...
				try {
					...
					console.error(
						`🔌 [PeerService] Heartbeat failed for session ${sessionId}:`,
						e,
					);
					this.unregisterSession(sessionId);
				}
	...
	registerSession(
		...
		for (const [existingId, existingSession] of this.activeSessions.entries()) {
			if (existingSession.userId === userId) {
				console.log(
					`🔌 [PeerService] Cleaning up stale session ${existingId} for user ${username} before re-register`,
				);
				try {
				...
				for (const [, res] of existingSession.pendingRequests.entries()) {
					try {
						res.status(503).json({ error: "Peer reconnected" });
					} catch {
						try {
							res.end();
						} catch {}
		...
		this.database.peer.createPeerSession(
			sessionId,
			userId,
			ipAddress,
			allowDownloads,
		...
		console.log(
			`🔌 [PeerService] Session ${sessionId} registered for user ${username}`,
		);
		return sessionId;
	}
	...
	handleManifest(sessionId: string, manifests: PeerTrackManifest[]) {
		...
		const peerTracks: Omit<PeerTrack, "session_id" | "created_at">[] =
			manifests.map((m) => ({
				id: m.id,
				title: m.title,
		...
		console.log(
			`🔌 [PeerService] Session ${sessionId} uploaded manifest with ${manifests.length} tracks`,
		);
	}
	...
	async requestStream(sessionId: string, trackId: string, res: Response) {
		...
		res.on("close", () => {
			if (session.pendingRequests.has(requestId)) {
				...
				try {
					if (session.ws.readyState === 1) {
						session.ws.send(
							JSON.stringify({ type: "cancel_request", requestId }),
						);
					}
				} catch {}
			}
		...
		try {
			...
			console.error(
				`🔌 [PeerService] Failed to send stream request to session ${sessionId}:`,
				e,
			);
			session.pendingRequests.delete(requestId);
	...
	async requestDownload(sessionId: string, trackId: string, res: Response) {
		...
		const globalAllowDownloads =
			this.database.identity.getSetting("peerAllowDownloads") !== "false";
		if (
			!globalAllowDownloads ||
			!session.allowDownloads ||
			!track.allow_download
		) {
			return res
				.status(403)
				.json({ error: "Downloads are disabled for this peer/track" });
		}
		...
		res.setHeader(
			"Content-Type",
			track.mime_type || "application/octet-stream",
		);
		if (track.file_size) {
			res.setHeader("Content-Length", track.file_size);
		}
		...
		let safeFilename =
			`${track.artist || "Unknown Artist"} - ${track.title || "Track"}.${track.mime_type === "audio/flac" ? "flac" : "mp3"}`
				.replace(/[<>:"/\\|?*\x00-\x1F]/g, "_")
				.trim();
		...
		try {
			...
			console.error(
				`🔌 [PeerService] Failed to send download request to session ${sessionId}:`,
				e,
			);
			session.pendingRequests.delete(requestId);
			res.status(500).json({ error: "Failed to communicate with peer" });
	...
	async requestImport(
		...
		trackId: string,
		destDir: string,
	): Promise<{ filePath: string; track: PeerTrack }> {
		let session = this.activeSessions.get(sessionId);
		let track = session
			? this.database.peer.getPeerTrack(sessionId, trackId)
			: undefined;
		...
		const ext =
			track.mime_type === "audio/flac"
				? "flac"
				: track.mime_type === "audio/wav"
					? "wav"
					: track.mime_type === "audio/ogg"
						? "ogg"
						: "mp3";
		let base = `${track.artist || "Unknown Artist"} - ${track.title || "Track"}`
			.replace(/[<>:"/\\|?*\x00-\x1F]/g, "_")
			.trim();
		if (base === "." || base === "..") base = "_";
		const filePath = path.join(
			destDir,
			`${base}-${String(track.id).slice(0, 8)}.${ext}`,
		);
		...
		const savedPath = await new Promise<string>((resolve, reject) => {
			...
			const pending: PendingImport = {
				...
				settled: false,
				timeout: setTimeout(
					() => this.failImport(requestId, "Import timed out"),
					5 * 60 * 1000,
				),
			};
			...
			stream.on("error", () =>
				this.failImport(requestId, "Failed to write file to disk"),
			);

			try {
				session.ws.send(
					JSON.stringify({ type: "download_request", requestId, trackId }),
				);
			} catch (e) {
	...
	handleChunk(
		...
		try {
			...
			console.error(
				`🔌 [PeerService] Failed to write chunk for request ${requestId}:`,
				e,
			);
	...
	handleChunkEnd(sessionId: string, requestId: string) {
		...
		if (importReq) {
			...
			console.log(
				`🔌 [PeerService] Import request ${requestId} finished successfully`,
			);
			return;
		}
		...
		session.pendingRequests.delete(requestId);
		console.log(
			`🔌 [PeerService] Request ${requestId} streaming finished successfully`,
		);
	}
	...
	handleChunkError(sessionId: string, requestId: string, message: string) {
		...
		if (this.pendingImports.has(requestId)) {
			console.error(
				`🔌 [PeerService] Import chunk error for request ${requestId}: ${message}`,
			);
		...
		console.error(
			`🔌 [PeerService] Chunk error from peer for request ${requestId}: ${message}`,
		);
		try {
			if (!res.headersSent) {
	...
	setPubkey(
		...
		for (const other of this.activeSessions.values()) {
			...
			if (other.ws.readyState === 1) {
				other.ws.send(
					JSON.stringify({ type: "pubkey", from: session.username, pubkey }),
				);
			}
			if (other.pubkey)
