...
export class LifecycleService {
    ...
    async finalizeRelease(id: number, user: { userId: number, artistId?: number, role: string }): Promise<void> {
        ...
        const existsInAlbums = rawDb ? !!rawDb.prepare("SELECT 1 FROM albums WHERE id = ?").get(id) : true;
        if (existsInAlbums) {
    ...
    async rejectPromotion(id: number, reason: string, admin: { userId: number, role: string }): Promise<void> {
        ...
        console.log(`[Lifecycle] Item ${id} rejected by Admin. Reason: ${reason}. Stored reason in curation_notes.`);
    }
}
