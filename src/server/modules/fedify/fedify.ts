...
import { getSiteHandle, isSiteHandle, SITE_ACTOR_ID } from "../../core/site-actor.js";
...
export function createFedify(dbService: DatabaseService, config: ServerConfig) {
    ...
    federation.setActorDispatcher("/users/{handle}", async (ctx, handle) => {
        ...
        const actorOptions = {
            ...
            url: new URL(isSiteHandle(handle, dbService) ? "/" : `/@${slug}`, baseUrl),
...
export function handleDeleteObject(
    ...
    if (actorUri && objectId === actorUri) {
        dbService.removeAllFollowers(actorUri as string);
        console.log(`🗑️ Remote actor deleted (optimized, single query); removed as follower: ${actorUri}`);
        return;
    }
