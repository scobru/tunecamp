...
export function createRssService(db: DatabaseService): RssService {
  ...
  return {
    async addFeed(url: string) {
      ...
      if (body === null)
        throw new Error(
          "Could not fetch the feed (unreachable, blocked, or not XML).",
        );
      ...
      console.log(
        `📡 [RSS] Followed "${parsed.channel.name}" (${parsed.items.length} items) — ${effectiveUrl}`,
      );
      return {
        name: parsed.channel.name || effectiveUrl,
    ...
    async refreshAll() {
      ...
      for (const feed of feeds) {
        try {
          ...
          console.error(
            `❌ [RSS] Refresh failed for ${feed.uri}: ${e?.message || e}`,
          );
        }
      }
    },
...
function parseFeed(xml: string, feedUrl: string): ParsedFeed | null {
  ...
  if (rssChannel.length) {
    const channel = {
      ...
      summary:
        text(childByTag(rssChannel, "description", "itunes:summary")) || null,
    ...
    $("item").each((_, el) => {
      ...
      const isAudio =
        !!enclosureUrl &&
        (enclosureType.startsWith("audio/") || AUDIO_EXT.test(enclosureUrl));
      ...
      items.push({
        ...
        content:
          text(childByTag(item, "content:encoded", "description", "summary")) ||
          null,
        url: link || null,
        cover_url: cover,
  ...
  if (atomFeed.length) {
    ...
    $("entry").each((_, el) => {
      ...
      entry.children().each((__, c) => {
        ...
        if ((node.tagName || (node as any).name || "").toLowerCase() !== "link")
          return;
      ...
      items.push({
        ...
        album_name: channel.name,
        duration: null,
        published_at: toIsoLocal(
          text(childByTag(entry, "published", "updated")),
        ),
      });
    });
