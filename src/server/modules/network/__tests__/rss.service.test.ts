...
describe("createRssService", () => {
  ...
  describe("addFeed", () => {
    it("should validate feed URL starts with http(s)://", async () => {
      await expect(
        rssService.addFeed("ftp://invalid.com/feed"),
      ).rejects.toThrow("Feed URL must start with http(s)://");
    });
    ...
    it("should handle unsafe URLs gracefully", async () => {
      ...
      ).rejects.toThrow("Could not fetch the feed");
    ...
    it("should parse a valid RSS feed and save channel and items", async () => {
      ...
      expect(mockDb.upsertRemoteContent).toHaveBeenNthCalledWith(
        1,
    ...
    it("should throw an error if no valid feed is found", async () => {
      (global.fetch as jest.Mock).mockResolvedValue({
        ...
        text: jest
          .fn()
          .mockResolvedValue("<html><body>No feed here</body></html>"),
      });

      await expect(
        rssService.addFeed("https://example.com/blog"),
      ).rejects.toThrow("The URL did not return a valid RSS or Atom feed");
    });
    ...
    it("should handle fetch failures", async () => {
      ...
      await expect(
        rssService.addFeed("https://example.com/feed"),
      ).rejects.toThrow("Could not fetch the feed");
    });
  });
  ...
  describe("refreshFeed", () => {
    it("should fetch and update metadata without changing follow state", async () => {
      ...
      expect(mockDb.upsertRemoteContent).toHaveBeenCalledWith(
      ...
      (global.fetch as jest.Mock)
        .mockResolvedValueOnce({
          ...
          text: jest
            .fn()
            .mockResolvedValue(
              "<rss><channel><item><guid>1</guid></item></channel></rss>",
            ),
        })
  ...
  describe("removeFeed", () => {
    it("should call db.unfollowActor with trimmed URL", () => {
      ...
      expect(mockDb.unfollowActor).toHaveBeenCalledWith(
