...
describe('decryptZenPrivHelper coverage test', () => {
    ...
    test('throws error for GCM with tampered auth tag', () => {
        ...
        expect(() => decryptZenPrivHelper(parts.join(':'), TESTING_SECRET)).toThrow();
    });
    ...
    test('throws error for CBC with wrong secret', () => {
        ...
        expect(() => decryptZenPrivHelper(legacyFormatEncrypted, 'invalid-secret')).toThrow();
    });

    test('throws error for GCM with invalid JSON data', () => {
        const key = crypto.createHash('sha256').update(TESTING_SECRET).digest();
        ...
        expect(() => decryptZenPrivHelper(encrypted, TESTING_SECRET)).toThrow();
    });

    test('throws error for CBC with invalid JSON data', () => {
        const key = crypto.createHash('sha256').update(TESTING_SECRET).digest();
        const iv = crypto.randomBytes(16);
        const cipher = crypto.createCipheriv('aes-256-cbc', key, iv);
        let dataHex = cipher.update('invalid-json-content', 'utf8', 'hex');
    ...
    test('throws error for completely invalid string formats', () => {
        ...
        expect(() => decryptZenPrivHelper('invalid:too:many:colons:here:yes', TESTING_SECRET)).toThrow();
    });
});
