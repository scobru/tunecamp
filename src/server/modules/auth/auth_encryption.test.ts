...
describe("Auth Encryption (Zen)", () => {
	...
	const LEGACY_ENCRYPTED =
		"111d819066de835cf73d01690cdef1b9:6e57757922c4edf5222e534e3febdecd863f04f344cf4b75fa8131473add56f7";
	...
	test("should fail to decrypt tampered data (GCM integrity check)", () => {
		...
		const tamperedData =
			originalData.slice(0, -1) + (originalData.slice(-1) === "0" ? "1" : "0");
		...
		const tamperedIv =
			originalIv.slice(0, -1) + (originalIv.slice(-1) === "0" ? "1" : "0");
		...
		const tamperedTag =
			originalTag.slice(0, -1) + (originalTag.slice(-1) === "0" ? "1" : "0");
	...
	test("should return null if decrypted data is not valid JSON (Legacy CBC)", () => {
		...
		expect(decryptZenPrivHelper(legacyNotJson, TEST_SECRET)).toBeNull();
	});
