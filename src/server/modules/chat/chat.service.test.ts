...
describe('ChatService', () => {
    ...
    describe('relayChat', () => {
        ...
        it('truncates text beyond the 2000 character cap', () => {
            ...
            const sent = JSON.parse((bobWs.send as jest.Mock).mock.calls[0][0] as string);
    ...
    describe('lobby history', () => {
        ...
        it('records a lobby message even when nobody else is connected to receive it', () => {
            ...
            expect(chatService.relayChat('alice-id', '', 'anyone there?')).toBe(false);
        ...
        it('trims the backlog to the 500-row cap', () => {
            ...
            const total = db.prepare('SELECT COUNT(*) AS n FROM peer_chat_messages').get() as { n: number };
        ...
        it('honours the requested limit', () => {
            ...
            expect(history.map(m => m.message)).toEqual(['msg-7', 'msg-8', 'msg-9']);
        });
        ...
        it('keeps relaying when the backlog write fails', () => {
            ...
            expect(chatService.relayChat('alice-id', '', 'still delivered')).toBe(true);
            expect(bobWs.send).toHaveBeenCalledWith(expect.stringContaining('still delivered'));
