 ...
 */
export async function sendBrevoEmail(config: ServerConfig, to: string, subject: string, htmlContent: string): Promise<void> {
    ...
    const res = await fetch("https://api.brevo.com/v3/smtp/email", {
        ...
        body: JSON.stringify({
            sender: { email: config.brevoSenderEmail, name: config.brevoSenderName || config.siteName || "TuneCamp" },
            to: [{ email: to }],
            subject,
            htmlContent,
        }),
