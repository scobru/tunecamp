...
export async function registerBuiltInDownloadProviders(
	...
	try {
		await syncRegistryWithDatabase(
			downloadService.getRegistry(),
			context.database,
		);
	} catch (e) {
	...
	return { cleanups, torrentService: undefined, ytdlpService: undefined };
}
