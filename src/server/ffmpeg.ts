 ...
 * Returns the path to the new MP3 file
 */
export async function convertWavToMp3(inputPath: string, bitrate: string | number = '320k'): Promise<string> {
    await acquireTaskSlot();
 ...
 */
export function transcode(inputPath: string, format: string = 'mp3', bitrate?: string | number, seek?: number): any {
 ...
 */
export async function writeMetadata(filePath: string, metadata: any): Promise<void> {
    await acquireTaskSlot();
    return new Promise((resolve, reject) => {
        ...
        if (metadata.track)
            command.outputOptions('-metadata', `track=${metadata.track}`);
        if (metadata.genre)
            command.outputOptions('-metadata', `genre=${metadata.genre}`);
        if (metadata.year)
            command.outputOptions('-metadata', `date=${metadata.year}`);
        command
            .save(tempPath)
            .on('end', async () => {
            releaseTaskSlot();
