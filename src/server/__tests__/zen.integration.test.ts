...
describe("Zen SEA Integration Routes", () => {
	...
	beforeEach(() => {
		...
		mockContainer.authService.updateUserProfile.mockImplementation(
			(username: string, updates: Record<string, any>) => {
				if (usersByUsername[username])
					Object.assign(usersByUsername[username], updates);
			},
		);
	...
	async function buildSsoToken(
		overrides: Record<string, any> = {},
		keys: FidKeyPair = baseKeys,
	) {
		const fields = {
	...
	test("POST /api/auth/zen/sso rejects missing ssoToken or apSeed", async () => {
		const res = await request(app).post("/api/auth/zen/sso").send({});
		expect(res.status).toBe(400);
	...
	test("POST /api/auth/zen/sso/exchange rejects a missing or unknown code", async () => {
		expect(
			(await request(app).post("/api/auth/zen/sso/exchange").send({})).status,
		).toBe(400);
