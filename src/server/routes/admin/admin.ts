...
import {
	...
	slugifySiteName,
	SITE_ACTOR_ID,
} from "../../core/site-actor.js";
import {
	getInstanceStorage,
	recomputeFileSizes,
	recomputeUserStorage,
} from "../../modules/storage/storage-usage.service.js";
...
const UPDATE_CHECK_URL =
	"https://raw.githubusercontent.com/scobru/tunecamp/main/package.json";
...
try {
	...
	/* dev layout without package.json in cwd; report 0.0.0 */
}
...
async function fetchLatestVersion(): Promise<string | null> {
	if (
		updateCheckCache &&
		Date.now() - updateCheckCache.fetchedAt < UPDATE_CHECK_TTL_MS
	) {
...
export function createAdminRoutes(container: ServiceContainer): Router {
	...
	const publishingService = container.publishingService;
	...
	const catalogCache: ServiceContainer["catalogCache"] = (container as any)
		.catalogCache;
	...
	router.use((req: AuthenticatedRequest, res, next) => {
		if (req.method !== "GET") {
			const canManage =
				req.context &&
				VisibilityGuardian.can(req.context, Capability.MANAGE_PRIVATE_LIBRARY);
			...
			const stripeOwnMatch = req.path.match(
				/^\/artists\/(\d+)\/stripe-connect/,
			);
			...
			const identityRefreshMatch = req.path.match(
				/^\/artists\/(\d+)\/refresh-identity$/,
			);
			...
			const canWrite =
				req.context && VisibilityGuardian.canWriteContent(req.context);
			...
			if (!canManage) {
				return res
					...
					.json({
						error:
							"Access denied: You do not have permission to modify administrative settings",
					});
			}
		}
		next();
	...
	router.get("/releases", (req: AuthenticatedRequest, res: any) => {
		try {
			...
			const canSeeAll =
				req.context &&
				VisibilityGuardian.can(req.context, Capability.VIEW_PRIVATE_LIBRARY);
			...
			if (showMine) {
				...
				const owned = req.userId
					? library
							.getReleasesByOwner(req.userId, VisibilityProfile.ALL_ACCESS)
							.map((r) => ({ ...r, is_formal_release: true }))
					: [];
				if (req.userId && req.artistId) {
					const artistReleases = library
						.getReleasesByArtist(req.artistId, VisibilityProfile.ALL_ACCESS)
						.map((r) => ({ ...r, is_formal_release: true }));
				...
				const formalReleases = library
					.getReleases(VisibilityProfile.ALL_ACCESS)
					.map((r) => ({ ...r, is_formal_release: true }));
				releases = [...formalReleases];

				// Only include library albums in promotion pipeline if explicitly requested (Curation Queue)
				if (includeLibrary) {
					const pendingAlbums = library
						.getAlbums(VisibilityProfile.ALL_ACCESS)
				...
				const ownedFormalReleases = library
					.getReleasesByOwner(req.userId, VisibilityProfile.ALL_ACCESS)
					.map((r) => ({ ...r, is_formal_release: true }));
				if (req.artistId) {
					const artistReleases = library
				...
				const ownedPendingAlbums = library
					.getAlbumsByOwner(req.userId, VisibilityProfile.ALL_ACCESS)
	...
	router.put(
		...
		async (req: AuthenticatedRequest, res: any) => {
			try {
				...
				const isAssociatedArtist =
					req.artistId !== undefined &&
					req.artistId !== null &&
					item.artist_id !== undefined &&
					item.artist_id !== null &&
					Number(item.artist_id) === Number(req.artistId);
				const canManageAll =
					req.context &&
				...
				if (
					...
					return res
						.status(403)
						.json({
							error: "Access denied: You can only manage your own content",
						});
				}

				const isNowPublic =
					newVisibility === "public" || newVisibility === "unlisted";
	...
	router.get("/stats", async (req: AuthenticatedRequest, res: any) => {
		try {
			...
			const isPrivileged =
				...
				VisibilityGuardian.can(req.context, Capability.MANAGE_ALL_CONTENT);
			const artistId =
				isPrivileged && !showMine ? undefined : req.artistId || undefined;
			...
			if (req.userId) {
				storageQuota =
					authService.getStorageInfo(req.userId)?.storage_quota || 0;
			}
	...
	router.get(
		...
		async (req: AuthenticatedRequest, res: any) => {
			...
			try {
				const overview = await getInstanceStorage(
					(database as any).db,
					musicDir,
				);
				res.json(overview);
	...
	router.get("/system/resources", (req: AuthenticatedRequest, res: any) => {
		...
		try {
			...
			/* DB file vanished or in-memory; report 0 */
		}
	...
	router.get(
		...
		async (req: AuthenticatedRequest, res: any) => {
			...
			res.json({
				...
				updateAvailable:
					latestVersion !== null &&
					compareSemver(latestVersion, localVersion) > 0,
	...
	router.put("/settings", async (req: AuthenticatedRequest, res: any) => {
		try {
			...
			if (
				...
				return res
					.status(403)
					.json({ error: "Only root admin can change site settings" });
			}
			...
			if (peerAllowDownloads !== undefined) {
				identity.setSetting(
					"peerAllowDownloads",
					isTrue(peerAllowDownloads) ? "true" : "false",
				);
			}
			...
			if (peerFederation !== undefined) {
				identity.setSetting(
					"peerFederation",
					isTrue(peerFederation) ? "true" : "false",
				);
			}
			...
			if (peerChatEnabled !== undefined) {
				identity.setSetting(
					"peerChatEnabled",
					isTrue(peerChatEnabled) ? "true" : "false",
				);
			}
			...
			if (peerChatGuestEnabled !== undefined) {
				identity.setSetting(
					"peerChatGuestEnabled",
					isTrue(peerChatGuestEnabled) ? "true" : "false",
				);
			}
			...
			if (boardEnabled !== undefined) {
				identity.setSetting(
					"boardEnabled",
					isTrue(boardEnabled) ? "true" : "false",
				);
				identity.setSetting(
					"chatEnabled",
					isTrue(boardEnabled) ? "true" : "false",
			...
			if (scheduledScanHour !== undefined) {
				...
				if (hour !== "" && (!/^\d{1,2}$/.test(hour) || Number(hour) > 23)) {
					return res
						...
						.json({
							error:
								"scheduledScanHour must be empty or an hour between 0 and 23",
						});
				}
			...
			if (publicUrl !== undefined) {
				if (publicUrl && !/^https?:\/\/.+\..+/.test(publicUrl)) {
					return res
						...
						.json({
							error:
								"publicUrl must be a valid URL with protocol (e.g. https://mysite.com)",
						});
				}
			...
			if (membershipMonthlyPrice !== undefined) {
				...
				if (!Number.isFinite(price) || price < 0) {
					return res
						.status(400)
						.json({
							error: "membershipMonthlyPrice must be a non-negative number",
						});
				}
			...
			if (trackcapTopupPriceUsd !== undefined) {
				...
				if (!Number.isFinite(price) || price < 0) {
					return res
						.status(400)
						.json({
							error: "trackcapTopupPriceUsd must be a non-negative number",
						});
				}
			...
			if (trackcapTopupTracksGranted !== undefined) {
				...
				if (!Number.isInteger(tracks) || tracks <= 0) {
					return res
						.status(400)
						.json({
							error: "trackcapTopupTracksGranted must be a positive integer",
						});
				}
			...
			if (allowPublicRegistration !== undefined) {
				identity.setSetting(
					"allowPublicRegistration",
					isTrue(allowPublicRegistration) ? "true" : "false",
				);
				settingsChanged = true;
			}
			...
			if (listenerSelfPublish !== undefined) {
				identity.setSetting(
					"listenerSelfPublish",
					isTrue(listenerSelfPublish) ? "true" : "false",
				);
			}
			...
			if (listenerSelfPublishQuota !== undefined) {
				...
				if (!Number.isFinite(quotaMb) || quotaMb < 0) {
					return res
						...
						.json({
							error:
								"listenerSelfPublishQuota must be a non-negative number (MB)",
						});
				}
				identity.setSetting(
					"listenerSelfPublishQuota",
					String(Math.floor(quotaMb)),
				);
			}
			...
			if (listenerTrackCap !== undefined) {
				...
				if (isNaN(trackCap) || trackCap < 0) {
					return res
						...
						.json({
							error:
								"listenerTrackCap must be a non-negative number (0 = unlimited)",
						});
				}
			...
			if (settingsChanged) {
				try {
					const finalName =
						identity.getSetting("siteName") ||
						config.siteName ||
						"TuneCamp Instance";
					const finalBio =
						identity.getSetting("siteDescription") ||
						config.siteDescription ||
						"Welcome to our TuneCamp community hub!";
					...
					if (siteName !== undefined) {
						...
						if (desiredHandle !== currentHandle) {
							...
							(database as any).db
								.prepare("UPDATE artists SET slug = ? WHERE id = ?")
								.run(desiredHandle, SITE_ACTOR_ID);
							console.log(
								`📡 [Settings] Site Actor handle updated: @${currentHandle} → @${desiredHandle}`,
							);
						}
					}
					...
					console.error(
						"❌ [Settings] Failed to sync Site Actor in artists table:",
						e,
					);
				}
			}
		...
		async (req: AuthenticatedRequest, res: any) => {
			try {
				if (!req.isAdmin) {
					return res
						.status(403)
						.json({ error: "Only admin can follow remote instances" });
				}
				const { url } = req.body;
				if (!url) {
				...
				res
					.status(500)
					.json({ error: error.message || "Failed to follow remote actor" });
			}
		},
	);
	 ...
	 */
	router.post(
		"/network/tunecamp/follow",
		async (req: AuthenticatedRequest, res: any) => {
			try {
				if (!req.isAdmin) {
					return res
						.status(403)
						.json({ error: "Only admin can follow instances" });
				}
				const { url } = req.body;
				...
				res.json({
					message: `Following TuneCamp instance ${origin}. Its catalog will appear in the Network.`,
				});
			} catch (error: any) {
				...
				res
					.status(500)
					.json({
						error: error.message || "Failed to follow TuneCamp instance",
					});
			}
		},
	);
		...
		async (req: AuthenticatedRequest, res: any) => {
			try {
				...
				if (!req.isRootAdmin) {
					return res
						.status(403)
						.json({ error: "Only root admin can access site identity" });
				}
	...
	router.post("/system/rescan", async (req: AuthenticatedRequest, res: any) => {
		try {
			...
			if (!maintenance) {
				return res
					.status(500)
					.json({ error: "Maintenance service not available" });
			}
				...
				const result = await scanner.scanDirectory(
					...
					(processed, total) => {
						taskManager.updateProgress(
							...
							processed,
							total,
							`Scanning library: ${processed}/${total} files`,
						);
					},
				);
				console.log(
					`✅ [Admin] Manual scan complete. Processed ${result.successful.length} files.`,
				);
				return {
			...
			if (!started) {
				return res
					.status(409)
					.json({ error: "Library scan is already in progress" });
			}

			res.json({
				message: "Library maintenance and scan triggered in background",
			});
		} catch (error) {
		...
		async (req: AuthenticatedRequest, res: any) => {
			try {
				if (!req.isRootAdmin) {
					return res
						.status(403)
						.json({ error: "Only root admin can sync tags" });
				}
				...
				const started = taskManager.run("tag-sync", () =>
					maintenance.syncAllTagsFromDb((processed, total) => {
						taskManager.updateProgress(
							"tag-sync",
							processed,
							total,
							`Synchronizing tags: ${processed}/${total} tracks`,
						);
					}),
				);
				...
				if (!started) {
					return res
						.status(409)
						.json({ error: "Tag synchronization is already in progress" });
				}

				res.json({
					message:
						"Tag synchronization started in background. This may take several minutes for large libraries.",
				});
			} catch (error) {
		...
		async (req: AuthenticatedRequest, res: any) => {
			try {
				if (
					...
					return res
						.status(403)
						.json({ error: "Only admin can prune orphan records" });
				}
		...
		async (req: AuthenticatedRequest, res: any) => {
			try {
				if (
					...
					return res
						.status(403)
						.json({ error: "Only admin can trigger cache pre-warming" });
				}

				const mediaEngine: ServiceContainer["mediaEngine"] = (container as any)
					.mediaEngine;
				if (!mediaEngine)
					return res.status(503).json({ error: "MediaEngine not available" });
				...
				console.log(
					`🔥 [Admin] Cache pre-warm triggered by ${req.username} — ${tracks.length} tracks`,
				);
				...
				taskManager.run("prewarm-cache", async () => {
					...
					const warmed = await mediaEngine.prewarmCache(
						tracks,
						(done, total, current) =>
							taskManager.updateProgress("prewarm-cache", done, total, current),
						() => aborted,
					);
				...
				res.json({
					message: `Pre-warm started for ${tracks.length} tracks`,
					taskId: "prewarm-cache",
				});
			} catch (error) {
		...
		async (req: AuthenticatedRequest, res: any) => {
			try {
				if (!req.isRootAdmin) {
					return res
						.status(403)
						.json({ error: "Only super root admin can sync community" });
				}
				...
				res
					.status(500)
					.json({ error: error.message || "Failed to sync community follows" });
			}
		},
	);
		...
		try {
			...
			if (!req.isRootAdmin && !req.isActive) {
				return res
					...
					.json({
						error:
							"Access denied: Account must be activated by admin to modify releases",
					});
			}
			...
			const isOwner =
				req.userId !== undefined &&
				(primaryOwnerId === req.userId || owners.includes(req.userId));
			const isAssociatedArtist =
				req.artistId !== undefined &&
				req.artistId !== null &&
				item.artist_id !== undefined &&
			...
			if (
				...
				console.warn(
					`⛔ [Debug] Access Denied for user ${req.username} on item ${id}. Owners: ${owners}, PrimaryOwner: ${primaryOwnerId}, Request UserId: ${req.userId}`,
				);
				return res
					.status(403)
					.json({
						error:
							"Access denied: Insufficient permissions to modify this item",
					});
			}
			...
			if (body.owner_id !== undefined && isPrivileged) {
				...
				updates.owner_id =
					parsedOwner != null && !Number.isNaN(parsedOwner)
						? parsedOwner
						: null;
			}
			...
			if (body.album_artist !== undefined)
				updates.album_artist = body.album_artist;
			if (body.albumArtist !== undefined)
				updates.album_artist = body.albumArtist;
			...
			const sellerArtist: any = sellerArtistId
				? library.getArtistSimple(Number(sellerArtistId))
				: null;
			const salesAllowed = !sellerArtist || sellerArtist.can_sell !== 0;
			if (body.price !== undefined)
				updates.price = salesAllowed ? body.price : 0;
			if (body.priceUsdc !== undefined)
				updates.price_usdc = salesAllowed ? body.priceUsdc : 0;
			if (body.priceUsdt !== undefined)
				updates.price_usdt = salesAllowed ? body.priceUsdt : 0;
			...
			if (genreToUse) {
				const genreStr = Array.isArray(genreToUse)
					? genreToUse.join(", ")
					: genreToUse;
				updates.genre = genreStr;
			}
			if (body.externalLinks)
				updates.external_links = JSON.stringify(body.externalLinks);
			...
			if (body.additional_artworks !== undefined) {
				updates.additional_artworks =
					typeof body.additional_artworks === "string"
			...
			database.transaction(() => {
				if (Object.keys(updates).length > 0) {
					if (release) {
						console.log(
							`   - Updating formal release metadata:`,
							Object.keys(updates),
						);
						library.updateRelease(id, updates);
					} else {
						console.log(
							`   - Updating library album metadata:`,
							Object.keys(updates),
						);
						...
						if (updates.visibility)
							library.updateAlbumVisibility(id, updates.visibility);
						if (updates.download !== undefined)
							library.updateAlbumDownload(id, updates.download);
						if (
							updates.price !== undefined ||
							updates.price_usdc !== undefined
						) {
							...
							if (curr) {
								...
								const pu =
									updates.price_usdc !== undefined
										? Number(updates.price_usdc)
										: (curr.price_usdc ?? 0);
								library.updateAlbumPrice(
									id,
									p,
									pu,
									(updates.currency || curr.currency || "ETH") as "ETH" | "USD",
								);
							}
						}
						if (updates.external_links)
							library.updateAlbumLinks(id, updates.external_links);
						if (updates.additional_artworks !== undefined)
							library.updateAlbum(id, {
			...
			if (body.track_ids && Array.isArray(body.track_ids)) {
				const newTrackIds = body.track_ids
					.map((tid: any) => parseInt(tid, 10))
					.filter((tid: any) => !isNaN(tid));
				console.log(
					`   - Received ${newTrackIds.length} track IDs from frontend for sync:`,
					newTrackIds,
				);
				...
				if (release) {
					database.transaction(() => {
						...
						console.log(
							`     🔄 Performing atomic track sync for formal release ${id}`,
						);
						...
						if (body.tracks_data && Array.isArray(body.tracks_data)) {
							console.log(
								`     📝 Updating track override metadata for formal release ${id}`,
							);
					...
					database.transaction(() => {
						...
						const toAdd = newTrackIds.filter(
							(ntid: number) => !existingTrackIds.includes(ntid),
						);
						const toRemove = existingTrackIds.filter(
							(etid: number) => !newTrackIds.includes(etid),
						);

						console.log(
							`   - Library Album Sync: toAdd=${toAdd.length}, toRemove=${toRemove.length}`,
						);
						...
						const trackOrders = newTrackIds.map(
			...
			publishingService
				.syncRelease(id)
				.catch((e) => console.error("❌ Failed to sync release update:", e));

			const finalItem = release ? library.getRelease(id) : library.getAlbum(id);
			console.log(
				`✅ [Debug] PUT /api/admin/releases/${id} completed successfully`,
			);
	...
	router.get(
		...
		async (req: AuthenticatedRequest, res: any) => {
			try {
				...
				const releaseDir = trackDir.includes("releases")
					? trackDir
					: path.join(musicDir, "releases", path.basename(trackDir));
					...
					await Promise.all(
						entries.map(async (entry) => {
							...
							if (entry.isDirectory()) {
								return walkDir(fullPath, `${prefix}${entry.name}/`);
							} else {
			...
			try {
				...
				if (!req.isAdmin && !req.isActive) {
					return res
						...
						.json({
							error:
								"Access denied: Account must be activated by admin to delete releases",
						});
				}
				...
				for (const id of ids) {
					...
					if (release && !req.isRootAdmin) {
						return res
							.status(403)
							.json({
								error: `Only Root Admin can delete formal release ${id}`,
							});
					}
					...
					if (
						...
						return res
							.status(403)
							.json({ error: `Access denied for item ${id}` });
					}
				...
				res.json({
					message: "Releases deleted successfully",
					count: allIds.length,
				});
			} catch (error) {
				...
				if (processedIds.length > 0) {
					...
					for (const id of processedIds) {
						publishingService
							.syncRelease(id)
							.catch((e) =>
								console.error(`Failed to sync batch visibility for ${id}:`, e),
							);
					}
				}
	...
	router.delete(
		...
		async (req: AuthenticatedRequest, res: any) => {
			try {
				...
				const item = (release ?? album) as {
					owner_id?: number | null;
					artist_id?: number | null;
				};
				if (
					!req.context ||
					!VisibilityGuardian.canManageItem(req.context, item)
				) {
					return res.status(403).json({ error: "Access denied" });
				}
			...
			try {
				...
				if (!isSystemAdmin && (!req.artistId || req.artistId !== artistId)) {
					return res
						...
						.json({
							error:
								"Access denied: Only the artist or root admin can access their identity keys",
						});
				}
			...
			try {
				...
				if (!isSystemAdmin && (!req.artistId || req.artistId !== artistId)) {
					return res
						...
						.json({
							error:
								"Access denied: Only the artist or root admin can refresh this identity",
						});
				}
	...
	router.get("/system/users", (req: AuthenticatedRequest, res: any) => {
		try {
			...
			if (
				!req.context ||
				!(req.role === "root_admin" || req.role === "admin")
			) {
				return res
					.status(403)
					.json({ error: "Only Root Admin or Manager can list users" });
			}
	...
	router.post("/system/users", async (req: AuthenticatedRequest, res: any) => {
		try {
			if (
				...
				return res
					.status(403)
					.json({ error: "Only the primary admin can create new admins" });
			}
			const {
				username,
				password,
			...
			const targetRole: UserRole =
				role || (isAdmin ? UserRole.ADMIN : UserRole.NORMAL_USER);
			const quota =
				storageQuota !== undefined
					? Number(storageQuota)
					: targetRole === UserRole.NORMAL_USER
						? 1024 * 1024 * 1024
						: 0;
		...
		async (req: AuthenticatedRequest, res: any) => {
			try {
				if (
					...
					return res
						.status(403)
						.json({ error: "Only the primary admin can manage users" });
				}
				const id = parseInt(req.params.id, 10);
				...
				const targetRole =
					...
					(isAdmin === undefined
						...
						: isAdmin
							...
							: "user");
				const quota =
					storageQuota !== undefined ? Number(storageQuota) : undefined;
	 ...
	 */
	router.post(
		"/system/users/:id/approve-artist",
		async (req: AuthenticatedRequest, res: any) => {
			try {
				if (
					!req.context ||
					...
					return res
						.status(403)
						.json({
							error: "Only the primary admin can approve artist requests",
						});
				}
				const id = parseInt(req.params.id, 10);
				...
				const quotaMb = Number(
					identity?.getSetting("listenerSelfPublishQuota"),
				);
				const quotaBytes =
					(Number.isFinite(quotaMb) && quotaMb >= 0 ? quotaMb : 1024) *
					1024 *
					1024;
				...
				res.json({
					success: true,
					artistId,
					message: `Artist profile created for ${user.username}`,
				});
			} catch (error: any) {
				console.error("Error approving artist request:", error);
				res
					.status(500)
					.json({ error: error.message || "Failed to approve artist request" });
			}
		},
	);
	...
	const getStripe = () => {
		const sKey =
			identity.getSetting("stripe_secret_key") || config.stripeSecretKey;
	 ...
	 */
	router.post(
		"/artists/:id/stripe-connect/onboard",
		async (req: AuthenticatedRequest, res: any) => {
			try {
				const artistId = parseInt(req.params.id, 10);
				if (!artistId || artistId < 0)
					return res.status(400).json({ error: "Invalid artist id." });
				if (!canManageConnect(req, artistId))
					return res.status(403).json({ error: "Access denied." });
				const stripe = getStripe();
				if (!stripe)
					return res
						.status(501)
						.json({ error: "Stripe not configured on this server." });
				...
				if (!accountId) {
					...
					console.log(
						`[Stripe Connect] Created Express account ${accountId} for artist ${artistId}`,
					);
				}

				const base = (
					identity.getSetting("publicUrl") ||
					config.publicUrl ||
					""
				).replace(/\/$/, "");
	 ...
	 */
	router.get(
		"/artists/:id/stripe-connect/status",
		async (req: AuthenticatedRequest, res: any) => {
			try {
				const artistId = parseInt(req.params.id, 10);
				if (!canManageConnect(req, artistId))
				...
				res
					.status(500)
					.json({ error: error.message || "Failed to fetch Stripe status." });
			}
		},
	);
	...
	router.delete(
		...
		(req: AuthenticatedRequest, res: any) => {
			try {
				...
				res
					.status(500)
					.json({ error: error.message || "Failed to unlink Stripe account." });
			}
		},
	);
	 ...
	 */
	router.delete(
		"/system/users/:id/artist-request",
		(req: AuthenticatedRequest, res: any) => {
			try {
				if (
					!req.context ||
					...
					return res
						.status(403)
						.json({
							error: "Only the primary admin can manage artist requests",
						});
				}
				const id = parseInt(req.params.id, 10);
				authService.setArtistRequest(id, false);
	...
	router.delete("/system/users/:id", (req: AuthenticatedRequest, res: any) => {
		try {
			if (
				...
				return res
					.status(403)
					.json({ error: "Only the primary admin can remove users" });
			}
			const id = parseInt(req.params.id, 10);
			...
			if (
				...
				error.message.includes("primary admin")
			) {
		...
		async (req: AuthenticatedRequest, res: any) => {
			try {
				if (
					...
					return res
						.status(403)
						.json({ error: "Only the primary admin can manage user status" });
				}
				const id = parseInt(req.params.id, 10);
				...
				res
					.status(500)
					.json({ error: error.message || "Failed to toggle user status" });
			}
		},
	);
		...
		try {
			...
			if (!req.isAdmin && !req.isActive) {
				return res
					...
					.json({
						error:
							"Access denied: Account must be activated by admin to modify posts",
					});
			}
			...
			if (updatedPost) {
				...
				publishingService
					.syncPost(id)
					.catch((e) => console.error("Failed to sync post update:", e));
			}
		...
		try {
			...
			if (
				...
				return res
					...
					.json({
						error:
							"Access denied: posting requires a Curator or Manager account",
					});
			}
			if (!req.isAdmin && !req.isActive) {
				return res
					...
					.json({
						error:
							"Access denied: Account must be activated by admin to create posts",
					});
			}
			...
			if (req.artistId && !req.isAdmin && req.artistId !== resolvedArtistId) {
				return res
					.status(403)
					.json({ error: "You can only post for your assigned artist" });
			}
			...
			if (resolvedArtistId === -1 && !isSystemAdmin) {
				return res
					.status(403)
					.json({ error: "Only root admin can post as site actor" });
			}

			const postId = social.createPost(
				resolvedArtistId,
				content,
				visibility || "public",
				title,
			...
			if (post) {
				...
				publishingService
					.syncPost(postId)
					.catch((e) => console.error("Failed to sync new post:", e));
			}
		...
		try {
			...
			if (!req.isAdmin && !req.isActive) {
				return res
					...
					.json({
						error:
							"Access denied: Account must be activated by admin to delete posts",
					});
			}
			...
			// Use PublishingService
			publishingService
				.unpublishPostFromAP(post)
				.catch((e) => console.error("Failed to sync post delete:", e));
			...
			try {
				...
				if (
					...
					return res
						...
						.json({
							error:
								"Access denied: publishing requires a Curator or Manager account",
						});
				}
				if (!req.isAdmin && !req.isActive) {
					return res.status(403).json({ error: "Access denied" });
				...
				const isSystemAdmin =
					req.context &&
					VisibilityGuardian.can(req.context, Capability.MANAGE_SYSTEM);
				const resolvedArtistId = artist_id
					? parseInt(artist_id)
					: req.artistId || null;
				if (
					!isSystemAdmin &&
					req.artistId &&
					resolvedArtistId !== req.artistId
				) {
					return res
						.status(403)
						.json({ error: "Cannot create assets for another artist" });
				}
				...
				const data: any = {
					...
					requires_subscription:
						requires_subscription === "true" || requires_subscription === true,
				};
		...
		async (req: AuthenticatedRequest, res: any) => {
			try {
				...
				if (body.artist_id !== undefined)
					updates.artist_id = parseInt(body.artist_id);
				if (body.price !== undefined) updates.price = parseFloat(body.price);
				if (body.price_usdc !== undefined)
					updates.price_usdc = parseFloat(body.price_usdc);
				if (body.currency !== undefined) updates.currency = body.currency;
				if (body.visibility !== undefined) updates.visibility = body.visibility;
				if (body.requires_subscription !== undefined)
					updates.requires_subscription =
						body.requires_subscription === "true" ||
						body.requires_subscription === true;
				if (req.file) {
	...
	router.get("/system/health", async (req: AuthenticatedRequest, res: any) => {
		...
		try {
			if (
				torrentService &&
				getDownloadService()?.getRegistry().isEnabled("torrent")
			) {
				const torrents = torrentService.getTorrentsStatus();
				const totalPeers = torrents.reduce(
					(sum: number, t: any) => sum + (t.numPeers || 0),
					0,
				);
				results.torrent = {
					connected: true,
				...
				results.torrent = {
					connected: false,
					error: torrentService ? "Disabled" : "Plugin not loaded",
				};
			}
		} catch (e) {
		...
		try {
			...
			const itunesRes = await fetch(
				"https://itunes.apple.com/search?term=jack+jackson&limit=1",
		...
		try {
			...
			const mbRes = await fetch(
				"https://musicbrainz.org/ws/2/release/?query=test&fmt=json",
				{
		...
		const discogsToken =
			identity.getSetting("discogs_token") || process.env.DISCOGS_TOKEN;
		...
		// 6. OpenRouter
		const orKey =
			identity.getSetting("openrouter_api_key") || config.openrouterApiKey;
		...
		results.openrouter = {
			...
			model:
				identity.getSetting("openrouter_model") ||
		...
		const stripeKey =
			identity.getSetting("stripe_secret_key") || config.stripeSecretKey;
		const stripeWebhook =
			identity.getSetting("stripe_webhook_secret") ||
		...
		// 10. Bandcamp
		try {
			const bcRes = await fetch("https://bandcamp.com", {
				method: "HEAD",
				signal: AbortSignal.timeout(5000),
			});
		...
		try {
			const controller = new AbortController();
			const timeout = setTimeout(() => controller.abort(), 5000);
			const deezerRes = await fetch("https://api.deezer.com/version", {
				signal: controller.signal,
			});
			clearTimeout(timeout);
		...
		results.lastfm = {
			configured: !!(
				identity.getSetting("lastfm_session_key") ||
				identity.getSetting("lastfm_api_key")
			),
		...
		const brevoEmail =
			identity.getSetting("brevo_sender_email") || config.brevoSenderEmail;
		results.brevo = {
		...
		// 15. Spotify
		try {
			const spotifyRes = await fetch("https://open.spotify.com", {
				method: "HEAD",
			});
		...
		async (req: AuthenticatedRequest, res: any) => {
			try {
				...
				const annotated = peers.map((peer: any) => {
					...
					const status = origin
						? federatedDiscoveryService.getInstanceStatus(origin)
						: null;
					return {
						...peer,
		...
		async (req: AuthenticatedRequest, res: any) => {
			try {
				if (!req.isAdmin) {
					return res
						.status(403)
						.json({ error: "Only admin can unfollow peers" });
				}
				const { url } = req.body;
				if (!url) {
					return res.status(400).json({ error: "URL is required" });
				...
				res
					.status(500)
					.json({ error: error.message || "Failed to unfollow remote actor" });
			}
		},
	);
			...
			try {
				...
				if (url) {
					...
					for (const peer of peers) {
						apService
							.fetchRemoteOutbox(peer.uri)
							.catch((e) => console.error(`Failed to sync ${peer.uri}:`, e));
					}
					rssService
						.refreshAll()
						.catch((e) => console.error("Failed to refresh RSS feeds:", e));
				...
				res
					.status(500)
					.json({ error: error.message || "Failed to sync remote actors" });
			}
		},
	);
			...
			try {
				if (!req.isAdmin) {
					return res
						.status(403)
						.json({
							error: "Only admin can purge the local remote-content cache",
						});
				}
				const publicUrl =
					identity.getSetting("publicUrl") ||
				...
				console.log(
					`🧹 [Admin] Purged ${deleted} stale local entries from remote_content`,
				);
				res.json({
					success: true,
					deleted,
					message: `Removed ${deleted} stale local post(s) from the federated cache.`,
				});
			} catch (error: any) {
				console.error("Error purging local remote-content cache:", error);
				res
					.status(500)
					.json({ error: error.message || "Failed to purge local cache" });
			}
		},
	);
	...
	router.post(
		...
		async (req: AuthenticatedRequest, res: any) => {
			try {
				...
				res.json({
					message: `Now following "${result.name}" (${result.items} items)`,
					...result,
				});
			} catch (error: any) {
				...
				res
					.status(400)
					.json({ error: error.message || "Failed to follow RSS feed" });
			}
		},
	);
	 ...
	 */
	router.post(
		"/network/rss/unfollow",
		async (req: AuthenticatedRequest, res: any) => {
			try {
				if (!req.isAdmin) {
					return res
						.status(403)
						.json({ error: "Only admin can unfollow feeds" });
				}
				const { url } = req.body;
				if (!url) {
					return res.status(400).json({ error: "URL is required" });
				...
				res
					.status(500)
					.json({ error: error.message || "Failed to unfollow RSS feed" });
			}
		},
	);
	...
	router.post(
		...
		async (req: AuthenticatedRequest, res: any) => {
			try {
				...
				res
					.status(500)
					.json({ error: error.message || "Failed to sync RSS feeds" });
			}
		},
	);
		...
		async (req: AuthenticatedRequest, res: any) => {
			try {
				if (!req.isAdmin) {
					return res
						.status(403)
						.json({ error: "Only admin can refresh peer catalogs" });
				}
				if (!catalogCache) {
				...
				res
					.status(500)
					.json({ error: error.message || "Failed to refresh peer catalogs" });
			}
		},
	);
	...
	router.get("/system/plugins", async (req: AuthenticatedRequest, res: any) => {
		...
		const flat = [
			...scanner
				.getRegistry()
				.getRegistryInfo()
				.map((p: any) => ({ ...p, type: "scanner" })),
			...metadataService
				.getRegistry()
				.getRegistryInfo()
				.map((p: any) => ({ ...p, type: "metadata" })),
			...streamingService
				.getRegistry()
				.getRegistryInfo()
				.map((p: any) => ({ ...p, type: "streaming" })),
			...(playlistService
				?.getRegistry()
				.getRegistryInfo()
			...
			...(scrobbleService
				?.getRegistry()
				.getRegistryInfo()
				.map((p: any) => ({ ...p, type: "scrobble" })) || []),
			...(getDownloadService()
				?.getRegistry()
				.getRegistryInfo()
				.map((p: any) => ({ ...p, type: "download" })) || []),
			...(aiService
				?.getRegistry()
				.getRegistryInfo()
				.map((p: any) => ({ ...p, type: "ai" })) || []),
	...
	router.put(
		...
		(req: AuthenticatedRequest, res: any) => {
			...
			if (!settings || typeof settings !== "object") {
				return res
					.status(400)
					.json({ error: "Body must be a JSON object of key-value pairs" });
			}
	...
	router.put(
		...
		async (req: AuthenticatedRequest, res: any) => {
			...
			if (typeof enabled !== "boolean") {
				return res
					.status(400)
					.json({ error: "Field 'enabled' (boolean) is required" });
			}
			...
			try {
				...
				res.json({
					message: `Plugin ${id} ${enabled ? "enabled" : "disabled"} successfully`,
					id,
					enabled,
				});
			} catch (error) {
