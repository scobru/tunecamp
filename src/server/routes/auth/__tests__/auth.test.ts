...
describe("Auth Routes", () => {
	...
	describe("POST /api/auth/password (integration, real auth service)", () => {
		test("persists the new password: old one stops working, new one authenticates", async () => {
            ...
            `);
			const realAuthService = createAuthService(
				db,
				"secret",
				"admin",
				"tunecamp",
			...
			const oldAuth = await realAuthService.authenticateUser(
				"admin",
				"tunecamp",
			);
			expect(oldAuth && oldAuth.success).toBeFalsy();

			const newAuth = await realAuthService.authenticateUser(
				"admin",
				"BrandNewPassword123!",
			);
	...
	describe("Artist Unlinking (integration, real auth service)", () => {
		test("prevents auto-linking on subsequent logins after explicit unlinking by root admin", async () => {
			...
			const artistInsert = db
				.prepare(
					"INSERT INTO artists (name, slug, visibility) VALUES ('curator1', 'curator1', 'public')",
				)
				.run();
			const artistId = Number(artistInsert.lastInsertRowid);

			// 2. Create curator1 admin user with no artist linked initially
			const curatorResult = await realAuthService.createUser(
				"curator1",
				"CuratorPass123!",
				null,
				1024,
			...
			const firstAuth = await realAuthService.authenticateUser(
				"curator1",
				"CuratorPass123!",
			);
			...
			let curatorRow = db
				.prepare("SELECT artist_id, artist_unlinked FROM admin WHERE id = ?")
				.get(curatorId) as any;
			...
			const secondAuth = await realAuthService.authenticateUser(
				"curator1",
				"CuratorPass123!",
			);
	...
	describe("POST /api/auth/login (zen_auth_mode)", () => {
		test("returns zenAuthMode on successful password login", async () => {
			mockAuthService.authenticateUser.mockResolvedValue({
				success: true,
				isAdmin: false,
				artistId: null,
			...
			mockAuthService.isRootAdmin.mockReturnValue(false);
			mockAuthService.isDefaultPassword.mockResolvedValue(false);
			mockAuthService.getUserByUsername.mockReturnValue({
				zen_auth_mode: "hybrid",
			});
			...
			expect(res.status).toBe(200);
			expect(res.body.zenAuthMode).toBe("hybrid");
		});

		test("rejects password login for ZEN-only account when password is provided", async () => {
			mockAuthService.authenticateUser.mockResolvedValue({
				success: false,
				zenAuthMode: "zen",
			});
			...
			const res = await request(app)
				.post("/api/auth/login")
				.send({ username: "zenuser", password: "Password123!" });

			expect(res.status).toBe(401);
		});
	});

	describe("GET /api/auth/status (zen_auth_mode)", () => {
		test("returns zenAuthMode in status response", async () => {
			const authApp = express();
			authApp.use(express.json());
			authApp.use((req: any, res, next) => {
				req.username = "testuser";
			...
			authApp.use(
				...
				createAuthRoutes({
					authService: {
						...mockAuthService,
						getUserByUsername: () => ({ zen_auth_mode: "hybrid" }),
					} as any,
					authMiddleware: mockAuthMiddleware,
				} as any),
			);
