...
- **Built-in defaults:** seeded on first boot in `database.ts` (currently 4-Track Recorder, Audiofabric, Iris, and Wormhole) — edit those `INSERT OR IGNORE` blocks to change the shipped defaults.
