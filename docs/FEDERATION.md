...
- **Downloadability is advertised, not assumed**: every release and track in `/api/catalog/full` carries a `downloadable` flag, evaluated as an anonymous stranger — the caller a federated fetch actually is. A release on sale, one published as streaming-only, or one pointing at an external store advertises `false`, and a client (a remote Network page, Sidecamp) offers no download for it. Streaming is unaffected either way; only the file transfer is withheld, and the seller's own download routes enforce it per request regardless of what a client does. An instance older than the flag still publishes its release's mode and price, which is enough for the consumer to apply the same policy locally.
