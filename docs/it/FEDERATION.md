...
- **La scaricabilità è dichiarata, non presunta**: ogni release e traccia in `/api/catalog/full` porta un flag `downloadable`, valutato come farebbe un estraneo anonimo — cioè il chiamante che una fetch federata è davvero. Una release in vendita, pubblicata come solo-streaming o che rimanda a uno store esterno dichiara `false`, e un client (una pagina Network remota, Sidecamp) non offre alcun download. Lo streaming non cambia in nessun caso: viene negato solo il trasferimento del file, e le rotte di download del venditore lo applicano comunque a ogni richiesta, qualunque cosa faccia il client. Un'istanza più vecchia del flag pubblica comunque modalità e prezzo della release, sufficienti al consumatore per applicare localmente la stessa policy.
