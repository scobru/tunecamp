...
- **Federazione delle tracce peer** (opt-in): quando un'istanza abilita *Federate Peer Tracks*, le sue tracce peer attualmente condivise viaggiano in `/api/catalog/full` e compaiono sulle pagine Network remote (con badge `PEER`), riproducibili — e, se i download sono consentiti, importabili — tra le istanze. Queste voci sono effimere e usano una finestra di cache breve, così spariscono rapidamente quando il peer si disconnette. Vedi [Condivisione Peer](./peer-sharing.md#federare-le-tracce-peer-tra-le-istanze).
