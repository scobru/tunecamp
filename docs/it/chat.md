...
- **`TUNECAMP_CHAT_FEDERATION_SECRET`** (variabile d'ambiente): Obsoleta. Non autentica più alcun messaggio in entrata; firma e verifica usano entrambe la chiave RSA di sito dell'istanza. Sopravvive solo come input di firma di ultima istanza per un'istanza a cui manca la coppia di chiavi di sito, e una firma del genere viene rifiutata da ogni peer. Il relay federato è disabilitato (`/inbound` risponde `503`) quando non c'è una `site_public_key` locale — vedi [Chat Federata](#chat-federata-cross-instance).
